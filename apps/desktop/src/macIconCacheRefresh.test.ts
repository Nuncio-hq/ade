...
describe("resolveLaunchVersionRecordPath", () => {
  it("places the record file inside the userData directory", () => {
    expect(resolveLaunchVersionRecordPath("/home/me/AppData/NuncioADE")).toBe(
      "/home/me/AppData/NuncioADE/last-launch-version.json",
    );
  });
});
...
describe("resolveMacAppBundlePath", () => {
  it("resolves the .app bundle from the Electron executable on macOS", () => {
    expect(
      resolveMacAppBundlePath("/Applications/NuncioADE.app/Contents/MacOS/NuncioADE", "darwin"),
    ).toBe("/Applications/NuncioADE.app");
  });

  it("returns null off macOS", () => {
    expect(
      resolveMacAppBundlePath("/Applications/NuncioADE.app/Contents/MacOS/NuncioADE", "linux"),
    ).toBeNull();
  });

  it("returns null when the executable is not inside a .app bundle", () => {
    expect(resolveMacAppBundlePath("/usr/local/bin/nuncioade", "darwin")).toBeNull();
  });
});
