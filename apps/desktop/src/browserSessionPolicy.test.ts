...
vi.mock("electron", () => ({
  app: {
    ...
    userAgentFallback:
      "Mozilla/5.0 AppleWebKit/537.36 Chrome/140.0.0.0 Electron/40.0.0 Safari/537.36 NuncioADE/0.5.5",
  },
  session: {
...
describe("BrowserSessionPolicy", () => {
  ...
  it("replaces identity headers case-insensitively without Electron product tokens", () => {
    ...
    expect(normalizedHeaders["user-agent"]).not.toMatch(/Electron|NuncioADE/iu);
  ...
  it("applies the same derived identity to the partition, tabs, and popups", () => {
    ...
    expect(partitionUserAgent).not.toMatch(/Electron|NuncioADE/iu);
