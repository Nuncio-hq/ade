...
describe("isWatchableBundlePath", () => {
  it("accepts a packaged asar app path", () => {
    expect(isWatchableBundlePath("/Applications/NuncioADE.app/Contents/Resources/app.asar")).toBe(
      true,
    );
  });

  it("rejects an unpackaged directory app path", () => {
    expect(isWatchableBundlePath("/Users/me/dev/nuncioade/apps/desktop")).toBe(false);
  });
});
