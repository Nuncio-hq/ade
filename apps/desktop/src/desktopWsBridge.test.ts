...
describe("desktopWsBridge", () => {
  ...
  it("reads only the canonical NuncioADE desktop URL environment value", () => {
    expect(
      resolveDesktopWsUrlFromEnv({
        NUNCIO_DESKTOP_WS_URL: "ws://127.0.0.1:6000/?token=nuncioade",
        UNRELATED_DESKTOP_WS_URL: "ws://127.0.0.1:5000/?token=ignored",
      }),
    ).toBe("ws://127.0.0.1:6000/?token=nuncioade");
    expect(
      resolveDesktopWsUrlFromEnv({
        UNRELATED_DESKTOP_WS_URL: "ws://127.0.0.1:5000/?token=ignored",
      }),
