...
import { MIGRATION_RECOVERY_MAX_RESUME_ATTEMPTS } from "@nuncio/shared/migrationRecovery";
...
describe("desktop migration recovery", () => {
  it("targets the same production database and bundled restore authority as the server", () => {
    expect(
      resolveDesktopMigrationRecoveryPaths({
        baseDir: Path.join(Path.sep, "home", "nuncioade"),
        appRoot: Path.join(Path.sep, "app"),
        isDevelopment: false,
      }),
    ).toEqual({
      dbPath: Path.join(Path.sep, "home", "nuncioade", "userdata", "state.sqlite"),
  ...
  it("uses the isolated development database when the desktop backend receives a dev URL", () => {
    ...
    expect(paths.dbPath).toBe(Path.join(Path.sep, "home", "nuncioade", "dev", "state.sqlite"));
  });

  it("continues only when the server-owned command clears the durable marker", async () => {
    const directory = await FS.mkdtemp(Path.join(OS.tmpdir(), "nuncioade-desktop-recovery-"));
...
describe("requiresDesktopMigrationRecovery", () => {
  async function withMarker(
    ...
    const directory = await FS.mkdtemp(Path.join(OS.tmpdir(), "nuncioade-recovery-gate-"));
    try {
      const paths = resolveDesktopMigrationRecoveryPaths({
        baseDir: directory,
        appRoot: directory,
