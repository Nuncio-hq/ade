...
const staticRoot = Path.resolve("/virtual/nuncioade-static");
...
describe("createDesktopStaticProtocolResolver", () => {
  it("resolves an existing asset inside the static root", () => {
    ...
    expect(resolveRequest("nuncioade://app/assets/app.js")).toEqual({ path: assetPath });
  });

  it("returns Electron file-not-found for a missing asset", () => {
    const resolveRequest = resolverWithExistingPaths([rootIndex]);

    expect(resolveRequest("nuncioade://app/assets/missing.js")).toEqual({ error: -6 });
  });
  ...
  it("resolves an extensionless route to its existing nested index", () => {
    ...
    expect(resolveRequest("nuncioade://app/settings")).toEqual({ path: nestedIndex });
  });

  it("falls back to the root index for a missing navigation route", () => {
    const resolveRequest = resolverWithExistingPaths([rootIndex]);

    expect(resolveRequest("nuncioade://app/thread/missing")).toEqual({ path: rootIndex });
  });
  ...
  it("keeps encoded traversal inside the root and safely handles malformed encoding", () => {
    ...
    expect(resolveRequest("nuncioade://app/..%2Foutside.js")).toEqual({ error: -6 });
    expect(resolveRequest("nuncioade://app/%E0%A4%A")).toEqual({ path: rootIndex });
    expect(
      observedPaths.every(
