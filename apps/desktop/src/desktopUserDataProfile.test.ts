...
function makeTempDir(): string {
  const directory = FS.mkdtempSync(Path.join(OS.tmpdir(), "nuncioade-profile-test-"));
...
describe("desktopUserDataProfile", () => {
  it("resolves the canonical NuncioADE profile names", () => {
    ...
    expect(
      resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "nuncioade-canary" }),
    ).toBe("/Users/tester/Library/Application Support/nuncioade-canary");
  });
  ...
  it("repairs missing browser data from the profile recorded by the bridge", () => {
    ...
    const targetPath = Path.join(appDataBase, "nuncioade");
    ...
    const targetPartitionPath = Path.join(targetPath, "Partitions", "nuncioade-browser");
    ...
    FS.writeFileSync(
      Path.join(targetPath, "nuncioade-profile-seed.json"),
  ...
  it("rejects bridge manifests that point outside the NuncioADE profile parent", () => {
    const appDataBase = makeTempDir();
  ...
  it("never adds a foreign SQLite sidecar beside an existing NuncioADE database", () => {
    const appDataBase = makeTempDir();
    const targetPath = Path.join(appDataBase, "nuncioade");
    const sourcePath = Path.join(appDataBase, "previous-profile");
    const sourcePartitionPath = Path.join(sourcePath, "Partitions", "previous-browser");
  ...
  it("replaces an orphaned target sidecar with one from the repaired database generation", () => {
    ...
    expect(
      FS.readdirSync(targetPartitionPath).some((entryName) =>
        entryName.startsWith(".nuncioade-bridge-"),
      ),
    ).toBe(false);
  });
  ...
  it("ignores a malformed bridge manifest without attempting a repair", () => {
    ...
    const targetPath = Path.join(appDataBase, "nuncioade");
    FS.mkdirSync(targetPath, { recursive: true });
    FS.writeFileSync(Path.join(targetPath, "nuncioade-profile-seed.json"), "{");
