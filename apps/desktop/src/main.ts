...
import type { ContextMenuItem } from "@nuncio/contracts";
import { isKeyboardShortcutsHelpChord } from "@nuncio/shared/browserShortcuts";
import { getMacTrafficLightPosition } from "@nuncio/shared/desktopChrome";
import {
...
import { applyShellEnvironmentHydrationMarker } from "@nuncio/shared/shell";
import { RotatingFileSink } from "@nuncio/shared/logging";
import { ensureStaticSnapshot, findAsarArchivePath } from "@nuncio/shared/staticSnapshot";
...
import {
  ...
  resolveNuncioADEStorageSnapshotPath,
...
// `BASE_DIR` prefers NUNCIO_HOME, which the Windows registry read hydrates whenever the
...
const desktopFlavor = resolveNuncioADEDesktopFlavor({
  isDevelopment,
  requestedFlavor: process.env.NUNCIO_DESKTOP_FLAVOR,
});
const desktopIdentity = nuncioadeDesktopIdentity(desktopFlavor);
const BASE_DIR =
  process.env.NUNCIO_HOME?.trim() ||
...
const BACKEND_MAX_OLD_SPACE_ENV_KEYS = ["NUNCIO_BACKEND_MAX_OLD_SPACE_MB"] as const;
...
const browserPerfLoggingEnabled = process.env.NUNCIO_BROWSER_PERF === "1";
...
function startBrowserPerformanceLogging(): void {
  ...
  browserPerfInterval = setInterval(() => {
    ...
    console.info(`[${NUNCIO_BROWSER_LABEL} perf]`, {
      ...snapshot.counters,
...
async function ensureBrowserUsePipeServer(): Promise<void> {
  if (browserUsePipeServer || !NUNCIO_BROWSER_USE_PIPE_PATH) {
    return;
  }
...
async function reserveBackendEndpoint(reason: string): Promise<void> {
  ...
  process.env.NUNCIO_DESKTOP_WS_URL = backendWsUrl;
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { nuncioadeCommitHash?: unknown };
    return normalizeCommitHash(parsed.nuncioadeCommitHash);
  } catch {
    return null;
  }
}

declare const __NUNCIO_WINDOWS_UPDATER_PUBLISHER__: string;
...
function resolveEmbeddedWindowsPublisherSubjects(): string[] {
  ...
  const subject = __NUNCIO_WINDOWS_UPDATER_PUBLISHER__.trim();
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.NUNCIO_COMMIT_HASH);
  if (envCommitHash) {
...
async function handleDesktopMigrationRecovery(): Promise<DesktopMigrationRecoveryOutcome> {
  ...
  const outcome = await recoverDesktopMigrationIfRequired({
    ...
    choose: async ({ previousFailure }) => {
      // The user is here because NuncioADE cannot open its database, so the
      ...
      if (canInstallUpdate) {
        choices.push({
          label: "Update NuncioADE and restart",
          detail: "install the newest NuncioADE release, which may already contain the fix",
      ...
      if (releaseUrl !== null) {
        choices.push({
          ...
          detail: `${canInstallUpdate ? "download that release" : "download the latest NuncioADE release"} in a browser`,
      ...
      const result = await dialog.showMessageBox({
        ...
        title:
          previousFailure === null
            ? "NuncioADE needs to recover its database"
            : restoreFailed
              ? "Migration recovery failed"
              : "NuncioADE could not update itself",
        message:
          previousFailure === null
            ? "NuncioADE stopped a database migration before it could finish safely."
            : restoreFailed
              ? "The saved database backup could not be restored."
              : "The newest NuncioADE release could not be installed.",
...
// nuncioade:// protocol here and, via NUNCIO_STATIC_DIR, for the backend's HTTP static
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("NuncioADE failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
...
function hasConfiguredUpdateFeed(): boolean {
  return readAppUpdateYml() !== null || Boolean(process.env.NUNCIO_DESKTOP_MOCK_UPDATES);
}
...
function resolveAutoUpdateDisabledReason(): string | null {
  return getAutoUpdateDisabledReason({
    ...
    disabledByEnv:
      desktopIdentity.usesScriptedUpdates || process.env.NUNCIO_DISABLE_AUTO_UPDATE === "1",
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `NuncioADE ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
    ...
    void dialog.showMessageBox({
      type: "info",
      title: "Update found",
      message: "NuncioADE is preparing the update in the background.",
      buttons: ["OK"],
    });
...
function resolveNotificationIconPath(): string | null {
  ...
  if (process.platform === "win32") {
    return resolveResourcePath("nuncioade.png") ?? resolveIconPath("ico");
  }
  return resolveResourcePath("nuncioade.png") ?? resolveIconPath("png");
}

function resolveAppSnapHelperPath(): string {
  if (app.isPackaged) {
    return Path.resolve(process.resourcesPath, "..", "Helpers", "nuncioade-appsnap-helper");
  }
  return Path.resolve(__dirname, "..", ".electron-runtime", "appsnap", "nuncioade-appsnap-helper");
}
 ...
 * package.json. We override it to a clean lowercase NuncioADE name.
 */
...
function repairBrowserProfileBeforeElectronReady(userDataPath: string): void {
  ...
  if (browserProfileRepair.status === "repaired") {
    console.info("[desktop] Completed NuncioADE browser profile bridge repair", {
    ...
    console.warn("[desktop] Failed to complete NuncioADE browser profile bridge repair", {
      sourcePath: browserProfileRepair.sourcePath,
      targetPath: browserProfileRepair.targetPath,
...
function configureAppIdentity(): void {
  ...
  app.setAboutPanelOptions({
    ...
    copyright: `© ${new Date().getFullYear()} Nuncio · based on NuncioADE by Emanuele Di Pietro`,
  });
...
function restartAfterStartupBundleSwap(error: BundleChangedDuringStartupError): void {
  ...
  void dialog
    .showMessageBox({
      type: "warning",
      title: "NuncioADE needs to restart",
      message: "NuncioADE changed while it was opening.",
      detail:
        "The current process cannot safely read the replaced application bundle. Restart NuncioADE to finish opening with one consistent version.",
      buttons: ["Restart NuncioADE"],
      defaultId: 0,
    })
    .catch(() => undefined)
    .then(() => {
...
// every archive read in this process — the nuncioade:// protocol, the backend's static
...
function startBundleSwapWatcher(): void {
  ...
  bundleSwapPollTimer = setInterval(() => {
    ...
    void dialog
      .showMessageBox({
        type: "warning",
        title: "NuncioADE was replaced on disk",
        message: "The installed NuncioADE app changed while it was running.",
        detail:
...
function processInstallMarkerOnStartup(): void {
  ...
  const message = `NuncioADE restarted, but update ${marker.toVersion} was not installed. Try again.`;
  setUpdateState(
    reduceDesktopUpdateStateOnInstallRestartFailure(
      updateState,
      marker.toVersion,
...
async function installLatestUpdateForMigrationRecovery(): Promise<string | null> {
  ...
  if (updateState.status === "up-to-date") {
    return `NuncioADE ${app.getVersion()} is already the newest release, so updating cannot repair this database.`;
  }
  if (updateState.status !== "downloaded") {
...
function configureAutoUpdater(): void {
  ...
  // default feed while NuncioADE versions advance independently.
  autoUpdater.channel = NUNCIO_DESKTOP_UPDATE_CHANNEL;
...
// Builds process-local Node args so provider/tool children do not inherit NuncioADE's heap guard.
...
function backendEnv(): NodeJS.ProcessEnv {
  ...
  const env: NodeJS.ProcessEnv = {
    ...resolveBrowserUsePipeBackendEnv(
      process.env,
      browserUsePipeServer ? NUNCIO_BROWSER_USE_PIPE_PATH : null,
    ),
    // Point the backend's HTTP static route at the same swap-immune snapshot the
    // nuncioade:// protocol serves, so both surfaces survive app.asar being replaced.
    ...(servedStaticRoot?.snapshotted ? { NUNCIO_STATIC_DIR: servedStaticRoot.dir } : {}),
    NUNCIO_MODE: "desktop",
    NUNCIO_NO_BROWSER: "1",
    NUNCIO_PORT: String(backendPort),
    NUNCIO_HOME: BASE_DIR,
    NUNCIO_AUTH_TOKEN: backendAuthToken,
    NUNCIO_DESKTOP_SHUTDOWN_TOKEN: DESKTOP_BACKEND_SHUTDOWN_TOKEN,
  };
...
function backendFailureDialogDetail(reason: string): string {
  ...
  return [
    cause,
    "NuncioADE paused automatic restarts so a failing backend can't keep respawning in the background.",
...
function presentBackendStartupGiveUp(reason: string): void {
  ...
  const task = (async () => {
    for (;;) {
      const result = await dialog.showMessageBox({
        type: "error",
        title: "NuncioADE's backend didn't start",
        message: `NuncioADE's backend failed to start ${BACKEND_MAX_CONSECUTIVE_START_FAILURES} times in a row.`,
        detail,
...
function handleBackendStartupBlock(block: BackendStartupBlock): void {
  ...
  const task = (async () => {
    if (block.kind === "migration-recovery-required") {
      const result = await dialog.showMessageBox({
        type: "warning",
        title: "NuncioADE needs to recover its database",
        message: "A database migration did not finish safely.",
        detail:
          "Restart NuncioADE to open the verified backup recovery flow. Provider and chat processes will remain stopped until recovery completes.",
    ...
    const processDetail =
      block.ownerPid === null
        ? "Another NuncioADE server is already using this database."
        : `Another NuncioADE server (process ${block.ownerPid}) is already using this database.`;
    const result = await dialog.showMessageBox({
      type: "warning",
      title: "NuncioADE is already running elsewhere",
      message: "Your local NuncioADE data is in use by another process.",
      detail: `${processDetail}\n\nStop the other NuncioADE app or development server, then try again. Your data has not been changed.`,
...
function startBackend(trigger: BackendStartTrigger = "lifecycle"): void {
  ...
  const child = ChildProcess.spawn(process.execPath, [...backendNodeArgs(), backendEntry], {
    ...
    env: {
      ...
      NUNCIO_SERVER_ENTRY: backendEntry,
    },
...
function getTitleBarOptions(): BrowserWindowConstructorOptions {
  ...
  return {
    ...
    // Derived from the shared chat-surface header geometry (@nuncio/shared/desktopChrome)
...
function presentRendererCrashRecovery(
  ...
  const message =
    response.cause === "reload-budget-exhausted"
      ? `NuncioADE's window crashed ${response.crashes} times in a row.`
      : "NuncioADE's window stopped unexpectedly.";
  const detail = [
    `The window's renderer process exited (${reason}).`,
    response.cause === "reload-budget-exhausted"
      ? "NuncioADE paused automatic reloads so a repeating crash can't keep reloading in the background."
      : "This exit reason repeats on reload, so NuncioADE did not retry automatically.",
  ...
  const task = (async () => {
    for (;;) {
      const result = await dialog.showMessageBox({
        type: "error",
        title: "NuncioADE's window stopped",
        message,
        detail,
...
async function bootstrap(): Promise<void> {
  ...
  try {
    ...
    console.warn("[NuncioADE browser] Failed to start browser-use native pipe", error);
  }
  startBackend();
