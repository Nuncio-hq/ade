...
// Purpose: Resolves NuncioADE's Electron userData paths and completes bridge profile repair.

import * as FS from "node:fs";
import * as OS from "node:os";
import * as Path from "node:path";

const BRIDGE_PROFILE_MANIFEST_FILE_NAME = "nuncioade-profile-seed.json";
const CANONICAL_BROWSER_PARTITION_NAME = "nuncioade-browser";
 ...
 *
 * The bridge manifest identifies the exact sibling profile that supplied the NuncioADE profile.
...
export function repairBrowserProfileFromBridgeManifest(
  ...
  try {
    ...
    for (const entryGroup of BROWSER_PARTITION_SEED_ENTRY_GROUPS) {
      ...
      const stagedGroupPath = FS.mkdtempSync(Path.join(targetPartitionPath, ".nuncioade-bridge-"));
