...
describe("electronUpdaterSecurity", () => {
  ...
  it("parses distinguished names the same way as builder-util-runtime", () => {
    const parsed = parseDistinguishedName('CN=NuncioADE, O="Acme, Inc.", OU=Tools\\2C Desktop');

    expect(parsed.get("CN")).toBe("NuncioADE");
  ...
  it("uses only embedded full publisher DNs and never feed-controlled names", () => {
    expect(
      resolveWindowsUpdatePublisherNames(
        ...
        [" CN=NuncioADE, O=Acme Tools ", "CN=Only", ""],
      ),
    ).toEqual(["CN=NuncioADE, O=Acme Tools"]);
  ...
  it("validates a matching full distinguished name with shell-free execFile options", async () => {
    const execFile = vi.fn((file, args, options, callback) => {
      callback(
        ...
        JSON.stringify({
          Status: 0,
          Path: "C:\\Users\\test\\AppData\\Local\\Temp\\NuncioADESetup.exe",
          SignerCertificate: {
            Subject: "CN=NuncioADE, O=Acme Tools",
          },
        }),
        "",
      );
    ...
    const result = await verifyWindowsUpdateCodeSignature(
      ["CN=NuncioADE, O=Acme Tools"],
      "C:\\Users\\test\\AppData\\Local\\Temp\\NuncioADESetup.exe",
  ...
  it("rejects a CN-only publisher allowlist", async () => {
      ...
      logger,
      {
        ...
        execFile: vi.fn((_file, _args, _options, callback) => {
          callback(
            ...
            JSON.stringify({
              Status: 0,
              Path: "C:\\Temp\\NuncioADESetup.exe",
              SignerCertificate: { Subject: "CN=NuncioADE, O=Acme Tools" },
            }),
            "",
          );
        }),
    ...
    expect(result).toContain("publisherNames: CN=NuncioADE");
  ...
  it("returns a mismatch summary for an unexpected publisher", async () => {
    ...
    expect(result).toContain("publisherNames: CN=NuncioADE, O=Acme Tools");
  ...
  it("patches electron-updater BaseUpdater spawnSyncLog only on Windows", () => {
    ...
    const prototype = FakeBaseUpdater.prototype as {
      ...
      __nuncioadeSpawnSyncLogPatched?: boolean;
    };
    ...
    expect(prototype.__nuncioadeSpawnSyncLogPatched).toBe(true);
  });
