...
describe("resolveElectronUpdaterCacheDirName", () => {
  it("matches electron-updater's cache directory fallback", () => {
    expect(resolveElectronUpdaterCacheDirName(null, "NuncioADE")).toBe("NuncioADE");
    expect(
      resolveElectronUpdaterCacheDirName({ updaterCacheDirName: "NuncioADE-updater" }, "NuncioADE"),
    ).toBe("NuncioADE-updater");
  });
});
...
describe("resolveElectronUpdaterPendingCacheDir", () => {
  it("matches electron-updater's pending cache path on macOS", () => {
    expect(
      resolveElectronUpdaterPendingCacheDir({
        cacheDirName: "NuncioADE-updater",
        platform: "darwin",
        homeDir: "/Users/test",
      }),
    ).toBe("/Users/test/Library/Caches/NuncioADE-updater/pending");
  });
  ...
  it("matches electron-updater's pending cache path on Windows", () => {
    ...
    ).toBe("C:\\Users\\test\\AppData\\Local\\NuncioADE-updater\\pending");
  });
  ...
  it("matches electron-updater's pending cache path on Linux", () => {
    ...
    ).toBe("/tmp/cache/NuncioADE-updater/pending");
  });
  ...
  it("falls back from an empty Linux cache env var like electron-updater", () => {
    ...
    ).toBe("/home/test/.cache/NuncioADE-updater/pending");
  });
