...
function makeIsolatedServerEnvironment(input: {
  ...
  return {
    ...environment,
    NUNCIO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "0",
    NUNCIO_CLAUDE_KEEPALIVE: "0",
    NUNCIO_DESKTOP_SHUTDOWN_TOKEN: input.shutdownToken,
    NUNCIO_HOME: input.homeDir,
    NUNCIO_HOST: "127.0.0.1",
    NUNCIO_MODE: "desktop",
    NUNCIO_NO_BROWSER: "1",
    NUNCIO_PORT: String(input.port),
  };
}
...
describeWindows("Windows desktop backend shutdown integration", () => {
  ...
  it("authenticates a real server, deduplicates shutdown, drains its runtime, and clears its finalizer artifact", async () => {
    ...
    try {
      const homeDir = await resources.makeTempDir("nuncioade-windows-shutdown-");
