...
export function remoteAccessPolicyError(
  ...
  if (config.publicUrl && !normalizeHttpsPublicOrigin(config.publicUrl)) {
    return "NUNCIO_PUBLIC_URL/--public-url must be an HTTPS root origin without credentials, path, query, or fragment (for example https://nuncioade.example.com).";
  }
  const isPubliclyExposed = isRemoteBind || Boolean(config.publicUrl);
  if (!isPubliclyExposed) return null;
  if (!config.authToken?.trim()) {
    return config.publicUrl
      ? "Refusing to publish NuncioADE through NUNCIO_PUBLIC_URL/--public-url without NUNCIO_AUTH_TOKEN/--auth-token."
      : `Refusing to bind NuncioADE to non-loopback host ${config.host ?? "<unspecified>"} without NUNCIO_AUTH_TOKEN/--auth-token.`;
  }
  if (config.devUrl) {
  ...
  if (isRemoteBind && !config.publicUrl && !config.allowInsecureRemote) {
    return "Refusing plaintext remote access. Configure an HTTPS reverse-proxy origin with NUNCIO_PUBLIC_URL/--public-url, or explicitly accept unencrypted LAN traffic with NUNCIO_ALLOW_INSECURE_REMOTE/--allow-insecure-remote.";
  }
  return null;
}
...
export function resolveDefaultChatWorkspaceRoot(input: {
  ...
  return pathApi.join(homeDir, "Documents", "NuncioADE");
}
...
export const resolveStaticDir = Effect.fn(function* () {
  ...
  const snapshotDir = process.env.NUNCIO_STATIC_DIR?.trim();
  if (snapshotDir) {
