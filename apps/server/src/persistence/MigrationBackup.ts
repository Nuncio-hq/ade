...
} from "@nuncio/shared/migrationRecovery";
export {
  migrationBackupDirectory,
  migrationRecoveryMarkerPath,
} from "@nuncio/shared/migrationRecovery";
...
export class MigrationRecoveryRequiredError extends Error {
  ...
  constructor(
    ...
    super(
      `Migration recovery is required for ${dbPath}.${detail ? ` ${detail}` : ""} Stop every NuncioADE process, then run: nuncioade-restore-migration-backup ${shellQuote(dbPath)}`,
    );
  ...
  constructor(
    ...
    super(
      `Not enough free disk space to back up the database before upgrading it. ` +
        ...
        `${formatBytes(availableBytes)} is free. Free up disk space and start NuncioADE again.`,
    );
...
const writeRecoveryMarker = (dbPath: string, backup: MigrationBackupResult) =>
  attemptPromise(async () => {
    await writeRecoveryMarkerFile(migrationRecoveryMarkerPath(dbPath), {
      ...
      resumeAttempts: 0,
      recovery:
        "NuncioADE retries an interrupted migration a bounded number of times on startup. Once that budget is spent it refuses to open this database until an operator stops every NuncioADE process and runs the explicit migration-backup restore command.",
    });
  });
...
/**
 * Explicit one-shot recovery path. The operator must stop every NuncioADE process
