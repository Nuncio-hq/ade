...
layer("032_ReconcileImportedSchemaLineage", (it) => {
  // Simulates a legacy ~/.nuncioade import where the imported `effect_sql_migrations`
  // tracker has IDs 17-31 recorded under unrelated NuncioADE names. The 17-31
  ...
  // references env_mode.
  it.effect("heals an imported NuncioADE DB whose tracker skipped 17-31", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;

      // Bring the schema to where NuncioADE and NuncioADE last agreed.
      yield* runMigrations({ toMigrationInclusive: 16 });

      // Mark IDs 17-31 applied under NuncioADE's old names so the migrator
      // skips NuncioADE's renumbered 17-23. Names are illustrative; only the
      ...
      // Seed a thread row with the NuncioADE-era column set so the data-rewrite
  ...
  it.effect("is a no-op on a fresh NuncioADE install", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
