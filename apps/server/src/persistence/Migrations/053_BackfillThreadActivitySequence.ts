...
export default Effect.gen(function* () {
  ...
  yield* sql`DROP TABLE IF EXISTS temp_nuncioade_activity_sequences`;

  yield* sql`
    CREATE TEMP TABLE temp_nuncioade_activity_sequences (
  ...
  yield* sql`
    INSERT INTO temp_nuncioade_activity_sequences (activity_id, sequence)
    SELECT
  ...
  yield* sql`
    ...
    SET sequence = (
      SELECT temp_nuncioade_activity_sequences.sequence
      FROM temp_nuncioade_activity_sequences
      WHERE temp_nuncioade_activity_sequences.activity_id = projection_thread_activities.activity_id
    )
  ...
  yield* sql`DROP TABLE temp_nuncioade_activity_sequences`;
});
