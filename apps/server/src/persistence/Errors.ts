...
export class MigrationLineageError extends Schema.TaggedErrorClass<MigrationLineageError>()(
  ...
  override get message(): string {
    return (
      ...
      `is recorded as "${this.recordedName}" but NuncioADE expects "${this.expectedName}". ` +
...
export class MigrationSchemaTooNewError extends Schema.TaggedErrorClass<MigrationSchemaTooNewError>()(
  ...
  override get message(): string {
    return (
      `Database schema migration ${this.databaseMigrationId} is newer than this NuncioADE build ` +
      `(latest supported migration: ${this.latestSupportedMigrationId}). ` +
      "Refusing writable startup; upgrade NuncioADE or restore a compatible database backup."
    );
  }
}
