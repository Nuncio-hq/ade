...
async function makeDbPath(): Promise<string> {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "nuncioade-sqlite-live-"));
  ...
  it("owns the live WAL exclusively without exposing a shared-memory sidecar", async () => {
    ...
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        yield* sql`INSERT INTO ownership_probe(value) VALUES ('owned-by-nuncioade')`;
        ...
        `;
        expect(rows).toEqual([{ value: "owned-by-nuncioade" }]);
        yield* Effect.promise(async () => {
          await expect(fs.stat(`${dbPath}-shm`)).rejects.toMatchObject({ code: "ENOENT" });
        });
      }).pipe(
