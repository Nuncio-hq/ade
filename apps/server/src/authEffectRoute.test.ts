...
import { AuthSessionId } from "@nuncio/contracts";
import {
...
function makeServerAuth(sideEffects: { count: number }): ServerAuthShape {
  ...
  return {
    ...
    authenticateHttpRequest: (request) => {
      ...
      const cookie = request.cookies.nuncioade_session === "cookie-token";
      if (!bearer && !cookie) {
    ...
    issueStartupPairingUrl: () =>
      Effect.succeed("https://nuncioade.example.test/pair#token=PAIRINGTOKEN"),
  ...
  return {
    ...
    headers: {
      ...
      ...(input.credential === "bearer"
        ...
        : { Cookie: "nuncioade_session=cookie-token" }),
  ...
  it("logs out either role and clears the exact cookie with secure public-mode attributes", async () => {
    ...
    await withAuthEffectServer(config, makeServerAuth(sideEffects), async (serverOrigin) => {
      const response = await fetch(
        ...
        mutationRequest({
          origin: "https://nuncioade.example.test",
          credential: "cookie",
        }),
      );
      ...
      expect(cookie).toContain("nuncioade_session=");
...
describe("binaryUploadEffectRouteLayer", () => {
  it("allows credentialed Canary attachment upload preflights", async () => {
    const config = {
      host: "127.0.0.1",
      attachmentsDir: fs.mkdtempSync(path.join(os.tmpdir(), "nuncioade-upload-cors-")),
    } as ServerConfigShape;
    try {
  ...
  it("rejects ambient cookie uploads without an origin and accepts explicit bearer auth", async () => {
    const attachmentsDir = fs.mkdtempSync(path.join(os.tmpdir(), "nuncioade-upload-route-"));
    const config = {
      host: "0.0.0.0",
      publicUrl: new URL("https://nuncioade.example.test/"),
      attachmentsDir,
      ...
      await withAuthEffectServer(
        ...
        async (serverOrigin) => {
          ...
          const cookieResponse = await fetch(url, {
            method: "POST",
            headers: { Cookie: "nuncioade_session=cookie-token" },
