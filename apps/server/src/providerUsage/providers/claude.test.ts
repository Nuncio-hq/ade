...
import { outboundHttp } from "@nuncio/shared/outboundHttp";
...
function makeClaudeHome(creds: Record<string, unknown>) {
  const homeDir = mkdtempSync(nodePath.join(os.tmpdir(), "nuncioade-claude-usage-"));
...
function makeClaudeConfigDir(creds: Record<string, unknown>) {
  const configDir = mkdtempSync(nodePath.join(os.tmpdir(), "nuncioade-claude-config-"));
