...
import { createCachedLoginShellPathReader } from "@nuncio/shared/loginShellEnvironment";
import {
  expandHomePath as expandHomePathSync,
  resolveNuncioADEHomeDirectory,
} from "@nuncio/shared/nuncioadeHome";
...
export function fixPath(
  ...
  try {
    ...
    // Cached by default: the probe result is persisted under the NuncioADE home and reused
...
export const resolveBaseDir = (raw: string | undefined): Effect.Effect<string> =>
  Effect.succeed(resolveNuncioADEHomeDirectory({ configuredHome: raw }));
