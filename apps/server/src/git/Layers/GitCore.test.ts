...
const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "nuncioade-git-core-test-",
});
  ...
  describe("stashAndCheckout", () => {
    ...
    it.effect("drops only the temporary stash it created after a successful reapply", () =>
      Effect.gen(function* () {
        ...
        expect(stashList).not.toContain("nuncioade: stash before switching to feature");
        expect(yield* readTextFile(path.join(tmp, "README.md"))).toBe("dirty changes\n");
      }),
    );
    ...
    it.effect("keeps the stash when reapplying dirty changes conflicts", () =>
      Effect.gen(function* () {
        ...
        expect(yield* git(tmp, ["stash", "list"])).toContain(
          "nuncioade: stash before switching to conflicting",
        );
      }),
    );
  });
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "nuncioade/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "nuncioade/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "nuncioade/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "nuncioade/tmp-working",
          newBranch: "nuncioade/feat/session",
        });

        expect(renamed.branch).toBe("nuncioade/feat/session-1");
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "nuncioade/feat/session")).toBe(
          true,
        );
        expect(branches.branches.some((branch) => branch.name === "nuncioade/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("nuncioade/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "nuncioade/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "nuncioade/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "nuncioade/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "nuncioade/tmp-working" });
        ...
        expect(renamed.branch).toBe("nuncioade/feat/session-2");
      }),
    );
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, ["remote", "add", "origin", "git@github.com:example-org/nuncioade.git"]);

          const remoteName = yield* core.ensureRemote({
            cwd: tmp,
            preferredName: "origin",
            url: "git@github.com:example-org/nuncioade.git/",
          });
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            yield* git(tmp, ["ls-remote", "--heads", "jasonLaster", "nuncioade/pr-488/statemachine"]),
          ).toBe("");
        }),
    );
