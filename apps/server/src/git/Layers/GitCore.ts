...
const AUTO_DETACHED_WORKTREE_DIRNAME = "nuncioade";
const WORKTREE_OWNERSHIP_MARKER = "nuncioade-agent-gateway-owner.json";
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `nuncioade-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
      ...
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const tempIndexDir = yield* fileSystem.makeTempDirectoryScoped({
            prefix: `nuncioade-git-status-index-${process.pid}-`,
          });
      ...
      Effect.gen(function* () {
        ...
        if (patch.length > 0) {
          yield* Effect.acquireUseRelease(
            Effect.tryPromise({
              try: () => nodeFs.mkdtemp(nodePath.join(tmpdir(), "nuncioade-worktree-patch-")),
              catch: (cause) =>
                createGitCommandError(
    ...
    const stashAndCheckout: GitCoreShape["stashAndCheckout"] = (input) =>
      Effect.gen(function* () {
        ...
        yield* executeGit(
          ...
          input.cwd,
          ["stash", "push", "-u", "-m", `nuncioade: stash before switching to ${input.branch}`],
          {
            timeoutMs: 30_000,
