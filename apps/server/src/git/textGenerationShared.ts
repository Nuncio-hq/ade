...
import { MAX_CHAT_THREAD_TITLE_WORDS } from "@nuncio/shared/chatThreads";
...
export function buildThreadRecapPrompt(input: {
  ...
  return {
    prompt: [
      "You are writing a compact live recap for NuncioADE's chat side panel.",
...
export function buildAutomationIntentPrompt(input: {
  ...
  return {
    prompt: [
      "You extract structured NuncioADE automation creation intents.",
      ...
      "- NuncioADE automations run a saved prompt on a schedule.",
...
export function buildAutomationCompletionEvaluationPrompt(input: {
  ...
  return {
    prompt: [
      "You evaluate whether a completed NuncioADE heartbeat automation should stop.",
