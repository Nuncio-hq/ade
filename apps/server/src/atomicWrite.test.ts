...
describe.skipIf(process.platform === "win32")("private atomic writes", () => {
  it("creates and replaces files with owner-only permissions", async () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "nuncioade-atomic-private-"));
  ...
  it("allows concurrent writers without sharing temporary files", async () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "nuncioade-atomic-concurrent-"));
    const filePath = path.join(directory, "state.json");
  ...
  it("replaces a final symlink without changing its outside target", async () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "nuncioade-atomic-symlink-"));
  ...
  it("rejects a symlinked parent directory without changing its outside target", async () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "nuncioade-atomic-parent-"));
  ...
  it("rejects a parent directory writable by other users", async () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "nuncioade-atomic-parent-mode-"));
    const filePath = path.join(directory, "state.json");
  ...
  it("enforces the requested mode even under a restrictive umask", async () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "nuncioade-atomic-umask-"));
    const filePath = path.join(directory, "state.json");
  ...
  it("repairs a newly created parent chain under a restrictive umask", async () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "nuncioade-atomic-parent-umask-"));
  ...
  it("finishes an in-flight commit before interruption is observed", async () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "nuncioade-atomic-interrupt-"));
  ...
  it("removes its temporary file when replacement fails", async () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "nuncioade-atomic-cleanup-"));
    const filePath = path.join(directory, "state.json");
