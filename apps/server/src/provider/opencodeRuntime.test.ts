...
import type { ChatAttachment } from "@nuncio/contracts";
...
describe("toOpenCodeFileParts", () => {
  it("materializes image attachments as SDK file parts", () => {
    ...
    expect(
      toOpenCodeFileParts({
        ...
        resolveAttachmentPath: () => "/tmp/nuncioade-attachments/screenshot.png",
      }),
    ).toEqual([
      {
        type: "file",
        ...
        url: "file:///tmp/nuncioade-attachments/screenshot.png",
      },
    ]);
  });
  ...
  it("leaves generic files for prompt-path projection", () => {
    ...
    expect(
      toOpenCodeFileParts({
        attachments: [attachment],
        resolveAttachmentPath: () => "/tmp/nuncioade-attachments/notes.docx",
      }),
    ).toEqual([]);
  });
});
...
describe("buildOpenCodeServerProcessEnv", () => {
  ...
  it("strips inherited NuncioADE authority from managed server processes", () => {
    const env = buildOpenCodeServerProcessEnv({
      baseEnv: {
        OPENAI_API_KEY: "provider-key",
        NUNCIO_AUTH_TOKEN: "server-secret",
        NUNCIO_BROWSER_USE_PIPE_PATH: "/tmp/browser.sock",
      },
    });

    expect(env.OPENAI_API_KEY).toBe("provider-key");
    expect(env.NUNCIO_AUTH_TOKEN).toBeUndefined();
    expect(env.NUNCIO_BROWSER_USE_PIPE_PATH).toBeUndefined();
  });
});
        ...
        Effect.gen(function* () {
          ...
          const first = yield* runtime
            .connectToOpenCodeServer({
              binaryPath: "opencode",
              cwd: "/repo",
              poolIsolationKey: "nuncioade-thread-a",
            })
            .pipe(Effect.provideService(Scope.Scope, firstScope));
          const second = yield* runtime
            .connectToOpenCodeServer({
              binaryPath: "opencode",
              cwd: "/repo",
              poolIsolationKey: "nuncioade-thread-b",
            })
            .pipe(Effect.provideService(Scope.Scope, secondScope));
