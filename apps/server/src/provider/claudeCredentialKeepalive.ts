...
//
// Opt in:   NUNCIO_CLAUDE_KEEPALIVE=1
// Tune:     NUNCIO_CLAUDE_KEEPALIVE_MINUTES=<n>   (default 30)
...
export function isClaudeCredentialKeepaliveEnabled(
  ...
  return platform === "darwin" && envFlagEnabled(env.NUNCIO_CLAUDE_KEEPALIVE);
}
...
export function resolveClaudeCredentialKeepaliveIntervalMs(env: NodeJS.ProcessEnv): number {
  const raw = env.NUNCIO_CLAUDE_KEEPALIVE_MINUTES?.trim();
...
export function startClaudeCredentialKeepalive(input?: {
  ...
  // NuncioADE should not do it as background work merely because the app opened.
