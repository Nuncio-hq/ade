...
import { NUNCIO_AGENT_GATEWAY_TOKEN_ENV } from "../../agentGateway/mcpInjection.ts";
...
describe("AcpNativeLogging", () => {
  it.effect("redacts gateway credentials from request and protocol NDJSON logs", () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nuncioade-acp-secret-log-"));
      ...
      try {
        ...
        yield* requestLogger({
          ...
          payload: {
            mcpServers: [
              ...
              {
                env: [
                  { name: NUNCIO_AGENT_GATEWAY_TOKEN_ENV, value: sentinelToken },
        ...
        yield* protocolLogger({
          ...
          payload: JSON.stringify({
            ...
            env: [{ name: NUNCIO_AGENT_GATEWAY_TOKEN_ENV, value: sentinelToken }],
          }),
        });
