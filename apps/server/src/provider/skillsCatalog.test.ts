...
import type { ProviderSkillDescriptor } from "@nuncio/contracts";
...
beforeEach(() => {
  clearSkillsCatalogCacheForTests();
  root = mkdtempSync(path.join(os.tmpdir(), "nuncioade-skills-catalog-"));
  homeDir = path.join(root, "home");
  nuncioadeBaseDir = path.join(homeDir, ".nuncioade");
});
...
describe("discoverSkillsCatalog", () => {
  it("creates the NuncioADE skills folder on first discovery", async () => {
    await discoverSkillsCatalog({ homeDir, nuncioadeBaseDir });
    await expect(access(path.join(nuncioadeBaseDir, "skills"))).resolves.toBeUndefined();
  });
  ...
  it("aggregates skills from nuncioade and provider home folders with origin scopes", async () => {
    ...
    const skills = await discoverSkillsCatalog({ homeDir, nuncioadeBaseDir });
    const byName = new Map(skills.map((skill) => [skill.name, skill]));

    expect(byName.get("portable")?.scope).toBe("nuncioade");
  ...
  it("can include duplicate skill names from different origins for settings", async () => {
    ...
    const defaultCatalog = await discoverSkillsCatalog({ homeDir, nuncioadeBaseDir });
  ...
  it("prefers the provider-native copy and falls back to NuncioADE for that provider", async () => {
    ...
    expect(codexView.some((skill) => skill.name === "only-nuncioade")).toBe(true);

    // A provider without its own copy resolves the NuncioADE fallback.
    ...
    expect(claudeShared?.scope).toBe("nuncioade");
  });

  it("uses documented provider alias roots before NuncioADE fallbacks", async () => {
    await writeSkill(path.join(nuncioadeBaseDir, "skills", "shared"), "shared", "NuncioADE copy");
...
`,
  ...
  it("serves cached results within the TTL and rescans on forceReload", async () => {
    await writeSkill(path.join(nuncioadeBaseDir, "skills", "first"), "first", "First skill");

    const initial = await discoverSkillsCatalog({ homeDir, nuncioadeBaseDir });
    expect(initial.map((skill) => skill.name)).toEqual(["first"]);

    // A skill added after the first scan is invisible to the cached entry...
    await writeSkill(path.join(nuncioadeBaseDir, "skills", "second"), "second", "Second skill");
    const cached = await discoverSkillsCatalog({ homeDir, nuncioadeBaseDir });
    expect(cached.map((skill) => skill.name)).toEqual(["first"]);

    // ...but forceReload bypasses the cache and refreshes it.
    const reloaded = await discoverSkillsCatalog({ homeDir, nuncioadeBaseDir, forceReload: true });
    expect(reloaded.map((skill) => skill.name).sort()).toEqual(["first", "second"]);
  });

  it("includes project-level .nuncioade skills when a cwd is provided", async () => {
    const cwd = path.join(root, "repo", "packages", "web");
    await mkdir(cwd, { recursive: true });
    await writeSkill(
      path.join(root, "repo", ".nuncioade", "skills", "repo-skill"),
      "repo-skill",
      "Project skill",
    );

    const skills = await discoverSkillsCatalog({ cwd, homeDir, nuncioadeBaseDir });
  ...
  it("keeps home origins when the cwd lives under the home dir", async () => {
    ...
    await writeSkill(path.join(nuncioadeBaseDir, "skills", "portable"), "portable", "NuncioADE");
    ...
    expect(skills.find((skill) => skill.name === "portable")?.scope).toBe("nuncioade");
  });

  it("dedupes same-named skills within a root deterministically", async () => {
    await writeSkill(path.join(nuncioadeBaseDir, "skills", "zeta"), "twin", "Copy in zeta");
    await writeSkill(path.join(nuncioadeBaseDir, "skills", "alpha"), "twin", "Copy in alpha");
...
describe("mergeSkillsIntoCatalog", () => {
  ...
  it("keeps provider-native entries and appends catalog-only entries", () => {
    const merged = mergeSkillsIntoCatalog({
      ...
      catalog: [descriptor("Shared", "nuncioade"), descriptor("extra", "nuncioade")],
    });
