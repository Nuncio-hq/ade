...
const nuncioadeSkillPath = "/Users/me/.nuncioade/skills/reviewer/SKILL.md";
...
describe("shouldInlineSkillForProvider", () => {
  it("skips codex-native and nuncioade roots for codex but inlines foreign provider roots", () => {
    // Codex loads .codex roots natively and ~/.nuncioade/skills via the extra
    // skill root registered at session start.
    expect(shouldInlineSkillForProvider("codex", nuncioadeSkillPath)).toBe(false);
  ...
  it("inlines only NuncioADE-owned paths for cursor", () => {
    expect(shouldInlineSkillForProvider("cursor", nuncioadeSkillPath)).toBe(true);
  ...
  it("inlines everything except .claude paths for claudeAgent", () => {
    ...
    expect(shouldInlineSkillForProvider("claudeAgent", nuncioadeSkillPath)).toBe(true);
  ...
  it("inlines cross-provider paths for pi but not pi-native skills", () => {
    expect(shouldInlineSkillForProvider("pi", nuncioadeSkillPath)).toBe(true);
  ...
  it("always inlines for providers without native skill support", () => {
    for (const provider of ["antigravity", "grok", "kilo", "opencode"] as const) {
      expect(shouldInlineSkillForProvider(provider, nuncioadeSkillPath)).toBe(true);
...
describe("buildInlineSkillInstructions", () => {
  it("inlines skill content for non-native providers and skips unreadable paths", async () => {
    ...
    const skillDir = path.join(root, ".nuncioade", "skills", "reviewer");
    try {
      ...
      const text = await buildInlineSkillInstructions({
        ...
        skills: [
          ...
          { name: "missing", path: path.join(root, ".nuncioade", "skills", "missing", "SKILL.md") },
        ],
        maxChars: 10_000,
      });
  ...
  it("does not inline nuncioade-rooted skills for codex (covered by the extra skill root)", async () => {
    const text = await buildInlineSkillInstructions({
      provider: "codex",
      skills: [{ name: "reviewer", path: nuncioadeSkillPath }],
      maxChars: 10_000,
    });
    expect(text).toBe("");
  });
