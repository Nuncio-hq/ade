...
describe("claudeProcessEnv", () => {
  ...
  it("does not grant NuncioADE control-plane authority to Claude", () => {
    const result = buildClaudeProcessEnv({
      env: {
        ANTHROPIC_API_KEY: "api-key-auth",
    ...
    assert.equal(result.ANTHROPIC_API_KEY, "api-key-auth");
    assert.equal(result.NUNCIO_AUTH_TOKEN, undefined);
    assert.equal(result.NUNCIO_BROWSER_USE_PIPE_PATH, undefined);
