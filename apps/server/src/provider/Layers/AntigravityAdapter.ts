...
export function buildAntigravityCaptureCommand(
  ...
  if (platform === "win32") {
    return `if not defined NUNCIO_ANTIGRAVITY_EVENTS (more >nul 2>nul & echo {}) else (set "ELECTRON_RUN_AS_NODE=1" && ${invocation})`;
  }
  return `if [ -z "\${NUNCIO_ANTIGRAVITY_EVENTS:-}" ]; then cat >/dev/null 2>&1 || :; printf '%s\\n' '{}'; else ELECTRON_RUN_AS_NODE=1 ${invocation}; fi`;
}
...
process.stdin.on("end", () => {
  const target = process.env.NUNCIO_ANTIGRAVITY_EVENTS;
  if (!target) {
  ...
  if (event === "pre-tool") {
    const decision = process.env.NUNCIO_ANTIGRAVITY_HOOK_DECISION === "allow" ? "allow" : "ask";
    ...
    `${JSON.stringify(
      {
        ...
        name: "nuncioade-capture",
        description: "Streams Antigravity CLI lifecycle events to NuncioADE when requested.",
      },
      null,
      2,
    )}\n`,
      ...
      yield* Effect.tryPromise({
        try: () => ensureCapturePlugin(binaryPath),
          ...
          new ProviderAdapterRequestError({
            ...
            detail: messageFromCause(cause, "Failed to install the NuncioADE capture hook."),
            cause,
          }),
      });
  ...
  const sendTurn: AntigravityAdapterShape["sendTurn"] = (input) =>
    Effect.gen(function* () {
      ...
      const runDir = yield* Effect.tryPromise({
        try: () => fs.mkdtemp(path.join(os.tmpdir(), "nuncioade-antigravity-")),
        catch: (cause) =>
          new ProviderAdapterRequestError({
            provider: PROVIDER,
            method: "turn/prepare",
      ...
      const child = spawn(context.binaryPath, args, {
        ...
        env: buildProviderChildEnvironment({
          provider: PROVIDER,
          inheritedNuncioADEKeys: ["NUNCIO_ANTIGRAVITY_EVENTS", "NUNCIO_ANTIGRAVITY_HOOK_DECISION"],
          overrides: {
            NUNCIO_ANTIGRAVITY_EVENTS: eventFile,
