...
export const takeDroidNuncioADEHarnessPolicyTextPart = (
...
const DROID_ACP_DEBUG_ENV = "NUNCIO_DROID_ACP_DEBUG";
...
// override with NUNCIO_DROID_TURN_IDLE_TIMEOUT_MS when a workload needs longer.
const DROID_TURN_IDLE_TIMEOUT_MS = resolveAcpTurnIdleTimeoutMs({
  envVar: "NUNCIO_DROID_TURN_IDLE_TIMEOUT_MS",
  defaultMs: 600_000,
});
...
const DROID_PLAN_MODE_PROMPT_PREFIX = [
  "NuncioADE Droid plan mode is active.",
...
interface DroidSessionContext {
  ...
  /** Turns cancelled by NuncioADE only because their Plan proposal was captured. */
  ...
  return Effect.gen(function* () {
        ...
        Effect.gen(function* () {
          ...
          if (resumeSessionId !== undefined && started.sessionSetupMethod === "new") {
            return yield* new ProviderAdapterRequestError({
              ...
              detail:
                "Droid could not resume the requested native session. NuncioADE refused the fresh fallback to avoid silently losing conversation context.",
            });
          }
    ...
    const startDroidTurn = (
      ...
      Effect.gen(function* () {
        ...
        const harnessPolicy = takeDroidNuncioADEHarnessPolicyTextPart(
          ctx,
          ...
          Effect.gen(function* () {
            if (!(yield* runtime.supportsSessionFork)) {
              return yield* new ProviderAdapterValidationError({
                ...
                operation: "forkThread",
                issue:
                  "This Droid ACP version does not advertise session/fork; NuncioADE will rebuild the fork from its retained transcript.",
              });
            }
        ...
        const forked = activeSource
          ...
          : yield* Effect.gen(function* () {
              ...
              const runtime = yield* makeDroidAcpRuntime({
                ...
                clientInfo: { name: "NuncioADE Fork", version: "0.0.0" },
              });
      ...
      discoveryLock.withPermits(1)(
        Effect.gen(function* () {
          ...
          const runtime = yield* makeDroidDiscoveryRuntime({
            ...(input.binaryPath ? { binaryPath: input.binaryPath } : {}),
            cwd,
            clientName: "NuncioADE Model Discovery",
          });
          yield* runtime.start();
      ...
      discoveryLock.withPermits(1)(
        Effect.gen(function* () {
          ...
          const runtime = yield* makeDroidDiscoveryRuntime({
            ...(input.binaryPath ? { binaryPath: input.binaryPath } : {}),
            cwd,
            clientName: "NuncioADE Command Discovery",
          });
          yield* runtime.start();
