...
// a native clipboard module, so importing it during NuncioADE startup can bloat the
  ...
  if (isRecord(result) && result.isError === true) {
    const message = Array.isArray(result.content)
      ? result.content
          ...
          .join("\n")
      : "";
    throw new Error(message || "NuncioADE gateway tool failed.");
  }
  const content =
...
const makePiAdapter = (options?: PiAdapterLiveOptions) =>
  Effect.gen(function* () {
    ...
    // Bridges the common Pi extension UI primitives onto NuncioADE's existing
    ...
    const makePiExtensionUIContext = (context: PiSessionContext): ExtensionUIContext => {
      ...
      const warnUnsupported = (method: string) => {
        ...
        offerRuntimeEvent({
          ...
          payload: {
            message: `Pi extension UI API '${method}' is not supported in NuncioADE yet.`,
            detail: { method },
          },
          raw: {
            source: "pi.sdk.event",
      ...
      // in ctx.ui`) to choose the NuncioADE path over the TUI dialog. Questions map
      ...
      const uiContext: ExtensionUIContext = {
        ...
        setTheme() {
          return { success: false, error: "NuncioADE does not expose Pi themes." };
        },
        getToolsExpanded() {
          return false;
        },
      ...
      Effect.gen(function* () {
          ...
          ? yield* releaseAgentGatewaySessionLeaseOnInterrupt(
              ...
              Effect.catch((cause) =>
                Effect.sync(() => agentGatewaySessionLease?.release()).pipe(
                  Effect.andThen(
                    Effect.logWarning(
                      "Pi could not install thread-scoped NuncioADE gateway tools",
                      cause,
                    ),
                  ),
        ...
        if (loadedExtensions.length > 0) {
          ...
          offerRuntimeEvent({
            ...
            payload: {
              message:
                "Pi extensions are loaded with NuncioADE's limited UI bridge. select/confirm/input/notify/status are supported; TUI-only widgets and editor hooks are ignored.",
              detail: {
    ...
    const respondUnsupported = (threadId: ThreadId, method: string) =>
      Effect.fail(
        new ProviderAdapterRequestError({
          provider: PROVIDER,
          method,
          detail: `Pi does not expose NuncioADE approval/user-input requests for thread ${threadId}.`,
        }),
      );
