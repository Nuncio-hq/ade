...
export const checkOmpProviderStatus = (
  ...
  Effect.gen(function* () {
    ...
    // OMP is SDK-backed in Synara. Keep this CLI probe advisory so health
    ...
    if (versionProbe.outcome === "missing" || versionProbe.outcome === "failure") {
      ...
      return {
        ...
        message:
          versionProbe.outcome === "missing"
            ? "OMP SDK is bundled, but the OMP CLI (`omp`) is not on PATH, so Synara could not verify the installed CLI version."
    ...
    if (versionProbe.outcome === "timeout") {
      return {
        ...
        checkedAt,
        message:
          "OMP SDK is bundled, but the CLI health check timed out before Synara could verify the installed version.",
      } satisfies ServerProviderStatus;
    }
    ...
    return {
      ...
      checkedAt,
      message: configuredAgentDir
        ? `OMP CLI is installed. Synara will use OMP agent dir ${configuredAgentDir}.`
