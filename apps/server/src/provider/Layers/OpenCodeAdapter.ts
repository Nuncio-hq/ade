...
import { buildOpenCodeMcpServer, NUNCIO_MCP_SERVER_NAME } from "../../agentGateway/mcpInjection.ts";
...
interface OpenCodeSessionContext {
  ...
  /** Permission request ids resolved by NuncioADE policy and never surfaced to the UI. */
    ...
    Effect.gen(function* () {
        ...
        if (!context.activeTurnToolCallIdleWatchdogStarted) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (context.activeTurnSawFinalAssistant) {
              ...
              if (
                ...
                (yield* recoverOpenCodeTurnFromMessageId(context, {
                  ...
                  raw: {
                    source: "nuncioade.opencode.deferred-idle-completion",
                    event: raw,
                  },
                }))
              ) {
              ...
              if (
                ...
                yield* completeOpenCodeTurn(context, {
                  turnId,
                  raw: {
                    source: "nuncioade.opencode.deferred-idle-local-part",
                    event: raw,
                  },
              ...
              if (
                ...
                (yield* recoverOpenCodeTurnFromMessageId(context, {
                  ...
                  raw: {
                    source: "nuncioade.opencode.deferred-idle-completion-retry",
                    event: raw,
                  },
                }))
              ) {
              ...
              if (
                ...
                yield* completeOpenCodeTurn(context, {
                  turnId,
                  raw: {
                    source: "nuncioade.opencode.deferred-idle-local-part-retry",
                    event: raw,
                  },
                  totalCostUsd: context.latestTurnCostUsd,
                });
            ...
            const completed = yield* completeOpenCodeTurn(context, {
              turnId,
              raw: {
                source: "nuncioade.opencode.idle-after-tool-calls",
                event: raw,
              },
      ...
      const recoverOpenCodeTurnFromMessages = Effect.fn("recoverOpenCodeTurnFromMessages")(
        function* (
          ...
          return yield* recoverOpenCodeTurnFromAssistantMessage(context, {
            ...
            assistantEntry,
            raw: {
              source: "nuncioade.opencode.prompt.recovery",
        ...
        yield* Effect.gen(function* () {
          ...
          Effect.flatMap(() =>
            Effect.gen(function* () {
              ...
              const completed = yield* completeOpenCodeTurn(context, {
                turnId: input.turnId,
                raw: { source: "nuncioade.opencode.prompt.watchdog" },
                errorMessage: message,
              });
              if (!completed) return;
              yield* emit(context, {
            ...
            Effect.gen(function* () {
              ...
              if (assistantEntry && isOpenCodeCompletedAssistantMessage(assistantEntry)) {
                yield* recoverOpenCodeTurnFromAssistantMessage(context, {
                  ...
                  assistantEntry,
                  raw: {
                    source: "nuncioade.opencode.prompt.response",
                    message: assistantEntry,
                  },
                });
              }
        ...
        switch (event.type) {
          ...
          case "permission.replied": {
            if (context.policyResolvedPermissionIds.delete(event.properties.requestID)) {
              // NuncioADE policy resolved this request; nothing was surfaced to the UI.
              break;
            }
          ...
          // agent loop. Mirror them into NuncioADE's canonical transcript stream.
      ...
      const startSession: OpenCodeAdapterShape["startSession"] = Effect.fn("startSession")(
        function* (input) {
          ...
          // server that this runtime isolates to the exact NuncioADE thread.
                  ...
                  Effect.gen(function* () {
                    ...
                    const gatewayControlAvailable = agentGatewayConnection
                      ? yield* runOpenCodeSdk("mcp.add", () =>
                          ...
                          Effect.flatMap((result) => {
                            const status = result.data?.[NUNCIO_MCP_SERVER_NAME];
                                  ...
                                  new OpenCodeRuntimeError({
                                    ...
                                    detail:
                                      status?.status === "failed"
                                        ? `${adapterConfig.displayName} NuncioADE MCP connection failed: ${status.error}`
                                        : `${adapterConfig.displayName} NuncioADE MCP connection did not become ready.`,
                                  }),
                                );
                          }),
                          Effect.as(true),
                          Effect.catchCause((cause) =>
                            Effect.sync(() => agentGatewaySessionLease?.release()).pipe(
                              Effect.andThen(
                                Effect.logWarning(
                                  `${adapterConfig.displayName} could not install thread-scoped NuncioADE MCP control`,
                                  Cause.squash(cause),
                                ),
                              ),
                              Effect.as(false),
                    ...
                    const createSessionId = resumedSessionId
                      ? // A resumed provider may still be executing an interrupted Plan turn.
                        // Install the read-only ruleset until NuncioADE dispatches a new turn with a
                        ...
                        // without ever emitting a permission request for NuncioADE to reject.
                      ...
                      : runOpenCodeSdk("session.create", () => {
                          const sessionCreateInput = {
                            ...
                            title: `NuncioADE ${input.threadId}`,
                          };
      ...
      const sendTurn: OpenCodeAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (input) {
        ...
        // Always pin NuncioADE's interaction mode to OpenCode's primary agent.
        // Otherwise a user config with default agent=plan (or a stale options.agent=plan
        // after leaving NuncioADE plan mode) can trap default turns in plan mode.
