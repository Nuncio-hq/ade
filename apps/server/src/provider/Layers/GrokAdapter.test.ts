...
import { TurnId } from "@nuncio/contracts";
...
import {
  ...
  takeGrokNuncioADEHarnessPolicyTextPart,
} from "./GrokAdapter.ts";

describe("Grok NuncioADE harness policy", () => {
  it("delivers private scoped host context once", () => {
    const state: { harnessPolicyDelivered?: boolean } = {};
    expect(takeGrokNuncioADEHarnessPolicyTextPart(state, true)?.text).toContain(
      NUNCIO_HARNESS_POLICY_MARKER,
    );
    expect(takeGrokNuncioADEHarnessPolicyTextPart(state, true)).toBeNull();
  });
});
...
describe("Grok native plan approval", () => {
  it("adds Plan instructions without sending a pager-only slash command as model text", () => {
    ...
    ).toMatch(/^NuncioADE requested Grok's native plan mode\./u);
  });
  ...
  it("backs native Plan mode with a fail-closed pre-tool hook", () => {
    expect(
      resolveGrokPlanHookResponse("plan", {
        hookCallbackId: "nuncioade-plan-guard",
  ...
  it("keeps native plan mode gated after NuncioADE captures the plan", () => {
    expect(makeGrokExitPlanModeCapturedResponse()).toEqual({
      outcome: "cancelled",
      feedback:
        "NuncioADE captured this plan for user review. Do not revise or implement it now. End this turn and wait for the user's next message.",
    });
  });
...
describe("Grok native user questions", () => {
  ...
  it("maps NuncioADE answers to Grok's question-text keyed response", () => {
