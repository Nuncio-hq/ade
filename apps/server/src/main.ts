...
const DESKTOP_SHUTDOWN_TOKEN_ENV_KEY = "NUNCIO_DESKTOP_SHUTDOWN_TOKEN";
...
interface CliInput {
  ...
  readonly nuncioadeHome: Option.Option<string>;
...
const CliEnvConfig = Config.all({
  mode: Config.string("NUNCIO_MODE").pipe(
    Config.option,
    Config.map(
  ...
  authToken: Config.string("NUNCIO_AUTH_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  desktopShutdownToken: Config.string("NUNCIO_DESKTOP_SHUTDOWN_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  ...
  logProviderEvents: optionalBooleanEnvironmentConfig("NUNCIO_LOG_PROVIDER_EVENTS"),
    ...
    Effect.gen(function* () {
      ...
      if (configuredPublicUrl && publicUrl === undefined) {
        return yield* new StartupError({
          message:
            "NUNCIO_PUBLIC_URL/--public-url must be an HTTPS root origin without credentials, path, query, or fragment (for example https://nuncioade.example.com).",
        });
      }
      ...
      const configuredHome = Option.getOrUndefined(input.nuncioadeHome) ?? env.nuncioadeHome;
...
const makeServerProgram = (input: CliInput) =>
  Effect.gen(function* () {
    ...
    yield* Effect.logInfo("NuncioADE running", makeServerStartupLogData(config));
      ...
      if (config.allowInsecureRemote && !config.publicUrl) {
        yield* Effect.logWarning(
          "INSECURE REMOTE ACCESS ENABLED: credentials and session traffic are unencrypted",
          {
            ...
            hint: "Use only on a trusted LAN. Configure NUNCIO_PUBLIC_URL behind HTTPS for protected remote access.",
          },
        );
      }
...
const nuncioadeHomeFlag = Flag.string("home-dir").pipe(
...
const publicUrlFlag = Flag.string("public-url").pipe(
  ...
  Flag.withDescription(
    "HTTPS public root origin provided by a TLS-terminating reverse proxy (equivalent to NUNCIO_PUBLIC_URL).",
  ),
  Flag.optional,
);
const allowInsecureRemoteFlag = optionalBooleanFlag("allow-insecure-remote", {
  description:
    "Explicitly allow unencrypted authenticated remote access on a trusted LAN (equivalent to NUNCIO_ALLOW_INSECURE_REMOTE).",
});
...
const logProviderEventsFlag = optionalBooleanFlag("log-provider-events", {
  description:
    "Emit native/canonical provider NDJSON logs for debugging (equivalent to NUNCIO_LOG_PROVIDER_EVENTS).",
});
const logWebSocketEventsFlag = optionalBooleanFlag("log-websocket-events", {
  description:
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to NUNCIO_LOG_WS_EVENTS).",
...
// Base `nuncioade` command defined before the MCP subcommands so they can yield
// its parsed input (notably `--home-dir` / `nuncioadeHome`) via Effect's command
...
const baseServerCommand = Command.make("nuncioade", {
  mode: modeFlag,
  port: portFlag,
  host: hostFlag,
  nuncioadeHome: nuncioadeHomeFlag,
  devUrl: devUrlFlag,
...
}).pipe(Command.withDescription("Run the NuncioADE server."));
...
const mcpServeCommand = Command.make(
  ...
  ({ integration }) =>
    Effect.gen(function* () {
      ...
      const baseDir = resolveExternalMcpBaseDir(Option.getOrUndefined(parent.nuncioadeHome));
  ...
  Command.withDescription(
    "Serve the paired NuncioADE external MCP integration over stdio for Codex, Claude, and other MCP clients.",
  ),
);
...
const mcpPairCommand = Command.make(
  "pair",
  {
    code: Flag.string("code").pipe(
      Flag.withDescription("Short-lived pairing code issued by NuncioADE Settings."),
    ),
  },
  ({ code }) =>
    Effect.gen(function* () {
      ...
      process.stdout.write(
        `Paired NuncioADE external MCP integration "${paired.paired.name}".\nCredential stored privately at ${paired.storePath}.\nConfigure the MCP client command as: ${externalMcpShellCommand(externalMcpLauncher(["mcp", "serve", "--integration", paired.paired.integrationId, "--home-dir", baseDir]))}\n`,
      );
      if (process.platform === "win32") {
        process.stdout.write(
          "Windows note: NuncioADE stores this credential under your user profile, but Windows does not expose POSIX 0600 permission checks. Protect the profile and its NuncioADE data directory.\n",
        );
      }
    }),
).pipe(Command.withDescription("Pair this CLI with a user-approved NuncioADE MCP integration."));

const mcpCommand = Command.make("mcp").pipe(
  Command.withDescription("Manage NuncioADE's loopback external MCP bridge."),
...
export const nuncioadeCli = serverCommand;
