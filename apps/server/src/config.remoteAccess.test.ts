...
describe("remote access policy", () => {
  ...
  it("requires authentication when an HTTPS proxy publishes a loopback bind", () => {
    ...
    ).toContain("without NUNCIO_AUTH_TOKEN");
  });

  it("rejects invalid public URLs in the shared embedded-server policy", () => {
    for (const publicUrl of [
      new URL("http://nuncioade.example.test/"),
      new URL("https://nuncioade.example.test/app"),
    ]) {
      expect(
        remoteAccessPolicyError({
          ...remoteBase,
  ...
  it("accepts only credential-free HTTPS root origins", () => {
    expect(normalizeHttpsPublicOrigin(new URL("https://nuncioade.example.test/"))?.origin).toBe(
      "https://nuncioade.example.test",
    );
    for (const value of [
      "http://nuncioade.example.test/",
      "https://user:pass@nuncioade.example.test/",
      "https://nuncioade.example.test/app",
      "https://nuncioade.example.test/?query=1",
      "https://nuncioade.example.test/#fragment",
    ]) {
