...
// Purpose: Compute Profile-page stats from NuncioADE's local projection DB only.
...
import { isBuiltInComposerSlashCommandName } from "@nuncio/shared/composerSlashCommands";
...
// Builds profile skill rows from every stored NuncioADE user message, plus the
...
>()("nuncioade/profileStats/ProfileStatsQuery") {}
...
const makeProfileStatsQuery = Effect.gen(function* () {
  ...
  // Activity = days/hours the user actually sent a NuncioADE prompt. One day-hour
  ...
  // Token usage for EVERY provider, straight from NuncioADE's own DB (no external
  ...
  const getProfileStats = (
    ...
    Effect.gen(function* () {
      ...
      // ── Identity ──
      const homeDirBasename = nodePath.basename(config.homeDir) || "nuncioade";
