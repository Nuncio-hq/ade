...
import type { AutomationDefinition, AutomationRun } from "@nuncio/contracts";
import { automationContinuesThread, automationOwnsItsThread } from "@nuncio/shared/automationMode";
...
// nuncioade_cancel_automation covers standalone runs, whose per-run thread owns nothing else.
const SELF_CANCEL_INSTRUCTION =
  "You may call nuncioade_cancel_automation on this automation when it is no longer needed.";
...
function reportingInstructions(mode: AutomationDefinition["mode"]): string {
  ...
  if (automationContinuesThread(mode)) {
    return [
      "Before finishing, call nuncioade_report_automation_result.",
      'Use decision "silent" when nothing needs the user\'s attention; otherwise use "notify".',
      SELF_CANCEL_INSTRUCTION,
    ].join(" ");
  }
  return [
    "Before finishing, call nuncioade_report_automation_result with a concise title and summary.",
...
export function buildAutomationRunEnvelope(input: {
  ...
  return [
    ...
    'Memory (persistent across runs — replace it via nuncioade_update_automation_memory {"memory": "..."} before finishing):',
