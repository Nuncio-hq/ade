...
describe("auth utils", () => {
  it("resolves stable web and port-scoped desktop cookie names", () => {
    expect(resolveSessionCookieName({ mode: "web", port: 3773 })).toBe("nuncioade_session");
    expect(resolveSessionCookieName({ mode: "desktop", port: 3773 })).toBe("nuncioade_session_3773");
  });
