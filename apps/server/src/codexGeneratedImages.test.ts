...
describe("resolveCodexGeneratedImagesRoot(s)", () => {
  ...
  afterEach(() => {
    if (previousNuncioADEHome === undefined) delete process.env.NUNCIO_HOME;
    else process.env.NUNCIO_HOME = previousNuncioADEHome;
  });

  it("returns the overlay generated_images directory as the active write root by default", () => {
    process.env.NUNCIO_HOME = "/nuncioade-test/runtime";
    assert.equal(
      resolveCodexGeneratedImagesRoot("/codex-test/.codex"),
      path.join("/nuncioade-test/runtime", "codex-home-overlay", "generated_images"),
    );
  });
  ...
  it("collapses to a single root when overlay equals source", () => {
    ...
    // The overlay falls under `<dirname(source)>/.nuncioade/runtime/codex-home-overlay`,
    ...
    const homePath = "/runtime/.nuncioade/runtime/codex-home-overlay";
