...
import { EDITORS } from "@nuncio/contracts";
...
it.layer(NodeServices.layer)("resolveEditorLaunch", (it) => {
  ...
  it.effect("adds the VS Code URL-handler trailing slash for existing folders", () =>
    Effect.gen(function* () {
      ...
      const dir = yield* fs.makeTempDirectoryScoped({ prefix: "nuncioade-vscode-folder-" });
  ...
  it.effect("opens terminal-style editors in the target working directory", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const dir = yield* fs.makeTempDirectoryScoped({ prefix: "nuncioade-open-terminal-" });
  ...
  it.effect("falls back to installed macOS app bundles when launchers are absent", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const home = yield* fs.makeTempDirectoryScoped({ prefix: "nuncioade-open-apps-" });
  ...
  it.effect("prefers the macOS Ghostty app launch even when a ghostty command is on PATH", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const home = yield* fs.makeTempDirectoryScoped({ prefix: "nuncioade-open-ghostty-" });
...
it.layer(NodeServices.layer)("launchDetached", (it) => {
  ...
  it.effect("rejects when command does not exist", () =>
    Effect.gen(function* () {
      const result = yield* launchDetached({
        command: `nuncioade-no-such-command-${Date.now()}`,
        args: [],
      }).pipe(Effect.result);
...
it.layer(NodeServices.layer)("isCommandAvailable", (it) => {
  it.effect("resolves win32 commands with PATHEXT", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const dir = yield* fs.makeTempDirectoryScoped({ prefix: "nuncioade-open-test-" });
  ...
  it.effect("uses platform-specific PATH delimiter for platform overrides", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const firstDir = yield* fs.makeTempDirectoryScoped({ prefix: "nuncioade-open-test-" });
      const secondDir = yield* fs.makeTempDirectoryScoped({ prefix: "nuncioade-open-test-" });
...
it.layer(NodeServices.layer)("resolveAvailableEditors", (it) => {
  it.effect("returns installed editors for command launches", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const dir = yield* fs.makeTempDirectoryScoped({ prefix: "nuncioade-editors-" });
  ...
  it.effect("returns VS Code when the Windows Store package is installed without a CLI", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const programFiles = yield* fs.makeTempDirectoryScoped({ prefix: "nuncioade-vscode-store-" });
  ...
  it.effect("does not treat Windows app-execution-alias folders as package installs", () =>
    Effect.gen(function* () {
      ...
      const localAppData = yield* fs.makeTempDirectoryScoped({
        prefix: "nuncioade-vscode-store-alias-",
      });
