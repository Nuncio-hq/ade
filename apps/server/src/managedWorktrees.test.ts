...
import type { OrchestrationThread } from "@nuncio/contracts";
...
async function makeManagedRoot(count: number) {
  const root = await fs.mkdtemp(path.join(tmpdir(), "nuncioade-managed-worktrees-"));
  temporaryRoots.push(root);
  const paths: string[] = [];
  for (let index = 0; index < count; index += 1) {
    const worktreePath = path.join(root, `task-${index}`, "nuncioade");
...
describe("managed worktrees", () => {
  ...
  it("matches threads whose recorded paths reach the worktree through a symlink", async () => {
    ...
    const linkRoot = await fs.mkdtemp(path.join(tmpdir(), "nuncioade-managed-worktrees-link-"));
