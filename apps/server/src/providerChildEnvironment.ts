...
// Purpose: Builds provider child environments without NuncioADE control-plane authority.
...
const isTestHarnessKey = (key: string, env: NodeJS.ProcessEnv): boolean =>
  Boolean(env.VITEST) && (key.startsWith("NUNCIO_FAKE_") || key.startsWith("NUNCIO_ACP_"));

export function buildProviderChildEnvironment(input: {
  readonly provider: ProviderChildKind;
  readonly baseEnv?: NodeJS.ProcessEnv;
  readonly inheritedNuncioADEKeys?: ReadonlyArray<string>;
  ...
  const allowedNuncioADEKeys = new Set(input.inheritedNuncioADEKeys ?? []);
  ...
  for (const [key, value] of Object.entries(baseEnv)) {
    if (
      key.startsWith("NUNCIO_") &&
      !allowedNuncioADEKeys.has(key) &&
