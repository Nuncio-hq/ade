...
describe("migration backup recovery CLI", () => {
  it("ships the recovery command from the bundled server package", async () => {
    ...
    expect(packageJson.bin?.["nuncioade-restore-migration-backup"]).toBe(
      "dist/restoreMigrationBackup.mjs",
    );
  });

  it("rejects relative database paths and warns operators to stop NuncioADE", async () => {
    ...
    expect(capture.errors.join("\n")).toContain("nuncioade-restore-migration-backup");
    expect(capture.warnings.join("\n")).toContain("Stop every NuncioADE process");
