...
import { CommandId, ThreadId } from "@nuncio/contracts";
...
export function recoverInterruptedAgentGatewayOperations(input: {
  ...
  readonly creationSource?: "nuncioade_mcp" | "external_mcp";
        ...
        Effect.gen(function* () {
          if (operation.status === "reserved") {
            yield* input.operationRepository.fail({
              ...
              errorJson: JSON.stringify({
                ...
                message:
                  "NuncioADE restarted before dispatch began. No git or orchestration resources were touched.",
              }),
              now: gatewayIsoNow(),
            });
            return;
            ...
            (entry) =>
              Effect.gen(function* () {
                ...
                if (Option.isSome(projected)) {
                  if (
                    projected.value.creationSource !== (input.creationSource ?? "nuncioade_mcp") ||
          ...
          if (recoveryErrors.length > 0) {
            yield* input.operationRepository.recordCompensationFailure({
              ...
              errorJson: JSON.stringify({
                ...
                message:
                  "NuncioADE could not fully compensate the interrupted operation during startup recovery. The sanitized operation remains retryable and some resources may require manual cleanup; no replacements will be created.",
          ...
          yield* input.operationRepository.fail({
            ...
            errorJson: JSON.stringify({
              ...
              message:
                "NuncioADE restarted before the operation completed. Deterministic operation-owned resources were compensated; no replacements were created.",
              ...
              yield* input.operationRepository
                .recordCompensationFailure({
                  ...
                  errorJson: JSON.stringify({
                    ...
                    message:
                      "NuncioADE could not recover the interrupted operation. The sanitized operation remains retryable and resources may require manual cleanup; no replacements will be created.",
                    error: detail,
                  }),
                  now: gatewayIsoNow(),
                })
