...
export function makeAgentGatewayAutomationTools(
  ...
  const createAutomation: ToolEntry = {
    ...
    definition: {
      ...
      description: `Create a heartbeat, standalone, or dedicated NuncioADE automation. ${AUTOMATION_AUTHORING_GUIDANCE} Existing calls remain compatible: omitting mode/schedule creates a heartbeat on your thread using everyMinutes (default 5). Prefer suggested:true unless the user explicitly requested creation.`,
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "Create a NuncioADE automation", ...WRITE_TOOL_ANNOTATIONS },
    },
  ...
  const listAutomations: ToolEntry = {
    ...
    definition: {
      name: "nuncioade_list_automations",
      description:
        "List NuncioADE automations (id, name, mode, schedule, target thread, enabled, next run).",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "List NuncioADE automations", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
    handler: (args) =>
      Effect.gen(function* () {
  ...
  const viewAutomation: ToolEntry = {
    ...
    definition: {
      ...
      description:
        "View a complete automation definition, recent runs, next run, and persistent-memory excerpt. Call this immediately before nuncioade_update_automation and resend every unchanged mutable field.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "View a NuncioADE automation", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
  ...
  const updateAutomation: ToolEntry = {
    ...
    definition: {
      ...
      description: `Fully replace an automation's mutable configuration. ${AUTOMATION_AUTHORING_GUIDANCE} You MUST call nuncioade_view_automation first, then resend name, prompt, schedule, enabled, maxIterations, notificationPolicy, and completionPolicy, including every unchanged field. Partial updates are rejected.`,
      inputSchema: {
        type: "object",
        properties: {
          automationId: { type: "string" },
      ...
      annotations: { title: "Replace a NuncioADE automation", ...WRITE_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
        const automationId = readStringArg(args, "automationId", { required: true })!;
  ...
  const cancelAutomation: ToolEntry = {
    ...
    definition: {
      ...
      description:
        'Stop a NuncioADE automation. mode "disable" (default) pauses it and keeps history; "delete" archives it. An automation-dispatched run may always stop its own automation, whatever its mode. Prefer a completionPolicy stop clause for conditions known when the automation is created.',
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "Stop a NuncioADE automation", ...WRITE_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
        const automationId = readStringArg(args, "automationId", { required: true })!;
  ...
  const reportResult: ToolEntry = {
    ...
    requiresActiveTurn: true,
    definition: {
      name: "nuncioade_report_automation_result",
      description:
