...
describe("agent gateway MCP injection", () => {
  it("builds a codex config block that references the token env var, not the token", () => {
    ...
    assert.include(block, "[mcp_servers.nuncioade]");
    assert.include(block, `url = "${connection.url}"`);
    assert.include(block, `bearer_token_env_var = "${NUNCIO_AGENT_GATEWAY_TOKEN_ENV}"`);
  ...
  it("appends the codex section once and keeps existing config intact", () => {
    ...
    assert.include(appended, "[mcp_servers.nuncioade]");

    const reappended = appendCodexConfigSection(appended, section);
    assert.equal(reappended.split("[mcp_servers.nuncioade]").length, 2);
  });
  ...
  it("merges the token exclusion into a user-defined shell environment policy", () => {
    ...
    const merged = mergeShellEnvPolicyExclude(withExclude, NUNCIO_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(merged, `exclude = ["${NUNCIO_AGENT_GATEWAY_TOKEN_ENV}", "AWS_*"]`);

    // Idempotent: the var is not added twice.
    assert.equal(mergeShellEnvPolicyExclude(merged, NUNCIO_AGENT_GATEWAY_TOKEN_ENV), merged);

    // A policy table without an exclude key gains one.
    const withoutExclude = ["[shell_environment_policy]", 'inherit = "core"'].join("\n");
    const gained = mergeShellEnvPolicyExclude(withoutExclude, NUNCIO_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(gained, `exclude = ["${NUNCIO_AGENT_GATEWAY_TOKEN_ENV}"]`);
    assert.include(gained, 'inherit = "core"');

    // No policy table: unchanged (the managed section appends its own).
    assert.equal(
      mergeShellEnvPolicyExclude('[model]\nname = "gpt-5.5"', NUNCIO_AGENT_GATEWAY_TOKEN_ENV),
      '[model]\nname = "gpt-5.5"',
    );
  });
  ...
  it("ignores commented and unrelated token references when merging shell exclusions", () => {
    const commentedExample = [
      "[shell_environment_policy]",
      `# exclude = ["${NUNCIO_AGENT_GATEWAY_TOKEN_ENV}"]`,
      'exclude = ["AWS_*"]',
    ].join("\n");
    ...
    assert.include(
      ...
      `exclude = ["${NUNCIO_AGENT_GATEWAY_TOKEN_ENV}", "AWS_*"]`,
    );

    const unrelatedString = [
      "[shell_environment_policy]",
      `note = "keep ${NUNCIO_AGENT_GATEWAY_TOKEN_ENV} private"`,
      'exclude = ["AWS_*"]',
    ].join("\n");
  ...
  it("recognizes only active exact entries in multiline shell exclusion arrays", () => {
    ...
    assert.equal(mergeShellEnvPolicyExclude(existing, NUNCIO_AGENT_GATEWAY_TOKEN_ENV), existing);
    ...
    const merged = mergeShellEnvPolicyExclude(tokenOnlyInComment, NUNCIO_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(merged, `exclude = ["${NUNCIO_AGENT_GATEWAY_TOKEN_ENV}",`);
  });

  it("detects real TOML table headers, ignoring comments and strings", () => {
    assert.isTrue(
      configHasTomlTableHeader('[mcp_servers.nuncioade]\nurl = "x"', "[mcp_servers.nuncioade]"),
    );
    assert.isTrue(
  ...
  it("uses the ACP http transport when the agent advertises support", () => {
    const servers = buildAcpNuncioADEMcpServers({
      connection,
    ...
    assert.deepEqual(servers, [
      {
        ...
        env: [
          { name: "NUNCIO_AGENT_GATEWAY_URL", value: connection.url },
          { name: NUNCIO_AGENT_GATEWAY_TOKEN_ENV, value: connection.bearerToken },
        ],
      },
    ]);
  });
