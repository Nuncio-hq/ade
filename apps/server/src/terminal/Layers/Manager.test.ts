...
describe("TerminalManager", () => {
  ...
  function makeManager(
    ...
    const logsDir = fs.mkdtempSync(path.join(os.tmpdir(), "nuncioade-terminal-"));
  ...
  it("keeps a running terminal alive when open reattaches with different cwd or env", async () => {
    ...
    const snapshot = await manager.open(
      openInput({ cwd: logsDir, env: { NUNCIO_TERMINAL_TEST: "changed" } }),
    );
  ...
  it("filters app runtime env variables from terminal sessions", async () => {
    ...
    try {
      ...
      expect(spawnInput.env.NUNCIO_PORT).toBeUndefined();
  ...
  it("injects runtime env overrides into spawned terminals", async () => {
    ...
    await manager.open(
      openInput({
        env: {
          NUNCIO_PROJECT_ROOT: "/repo",
          NUNCIO_WORKTREE_PATH: "/repo/worktree-a",
          CUSTOM_FLAG: "1",
        },
      }),
    );
    ...
    expect(spawnInput.env.NUNCIO_PROJECT_ROOT).toBe("/repo");
    expect(spawnInput.env.NUNCIO_WORKTREE_PATH).toBe("/repo/worktree-a");
