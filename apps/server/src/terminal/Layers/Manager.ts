...
function shouldStripOscSequence(content: string): boolean {
  return (
    /^(10|11|12);(?:\?|rgb:)/.test(content) || content.startsWith(NUNCIO_TERMINAL_HOOK_OSC_PREFIX)
  );
}
...
function extractOscHookEvent(content: string): TerminalAgentHookEventType | null {
  if (!content.startsWith(NUNCIO_TERMINAL_HOOK_OSC_PREFIX)) {
    return null;
  }
  const eventType = content.slice(NUNCIO_TERMINAL_HOOK_OSC_PREFIX.length).trim();
...
function shouldExcludeTerminalEnvKey(key: string): boolean {
  ...
  if (normalizedKey.startsWith("NUNCIO_")) {
    return true;
  }
...
function cliKindFromRuntimeEnv(
  ...
  return terminalCliKindFromValue(runtimeEnv?.[NUNCIO_TERMINAL_CLI_KIND_ENV_KEY]);
}
