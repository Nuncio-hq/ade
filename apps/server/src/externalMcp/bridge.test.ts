...
function makeBaseDir() {
  const value = fs.mkdtempSync(path.join(os.tmpdir(), "nuncioade-mcp-bridge-test-"));
...
describe("external MCP stdio bridge", () => {
  ...
  it("passes hostile Windows runtime paths outside the encoded PowerShell program", () => {
    ...
    expect(invocation.options.env.NUNCIO_RUNTIME_ACL_TARGET).toBe(targetPath);
    ...
    expect(
      requestTimeoutForBody(
        JSON.stringify({
          ...
          id: 1,
          method: "tools/call",
          params: { name: "nuncioade_wait_for_task", arguments: { threadId: "thread-1" } },
        }),
      ),
    ).toBe(35_000);
    expect(
      ...
      requestTimeoutForBody(
        JSON.stringify([
          ...
          },
          {
            ...
            id: 4,
            method: "tools/call",
            params: { name: "nuncioade_wait_for_task", arguments: { threadId: "thread-2" } },
          },
        ]),
      ),
    ).toBe(65_000);
    expect(
      requestTimeoutForBody(
        JSON.stringify({
          ...
          id: 5,
          method: "tools/call",
          params: { name: "nuncioade_create_task", arguments: {} },
        }),
      ),
    ).toBe(605_000);
  });
    ...
    await serveExternalMcpStdio({
      ...
      fetchImpl: async (url, init) => {
        ...
        if (urls.length === 1) {
          ...
          throw new TypeError("old NuncioADE instance stopped");
        }
  ...
  it("allows a fast request to complete while a wait request is still pending", async () => {
    ...
    await serveExternalMcpStdio({
      baseDir,
      stdin: Readable.from([
        `${JSON.stringify({ jsonrpc: "2.0", id: "slow", method: "tools/call", params: { name: "nuncioade_wait_for_task", arguments: { timeoutMs: 100 } } })}\n`,
  ...
  it("aborts an active HTTP request when the MCP client cancels its request id", async () => {
    ...
    stdin.write(
      `${JSON.stringify({ jsonrpc: "2.0", id: "slow", method: "tools/call", params: { name: "nuncioade_wait_for_task", arguments: { threadId: "thread-1" } } })}\n`,
    );
