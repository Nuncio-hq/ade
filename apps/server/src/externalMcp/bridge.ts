...
const WINDOWS_RUNTIME_ACL_SCRIPT = [
  ...
  "$target = $env:NUNCIO_RUNTIME_ACL_TARGET",
...
export function makeWindowsRuntimeAclPowerShellInvocation(targetPath: string) {
  return {
    ...
    options: {
      ...
      maxBuffer: 1024 * 1024,
      timeout: 5_000,
      env: { ...process.env, NUNCIO_RUNTIME_ACL_TARGET: targetPath },
    },
  };
}
...
class ExternalMcpRequestTimeoutError extends ExternalMcpBridgeError {
  constructor(timeoutMs: number) {
    super(`NuncioADE did not respond within ${timeoutMs} ms.`);
...
export function resolveExternalMcpBaseDir(homeDir?: string): string {
  const configured = homeDir?.trim() || process.env.NUNCIO_HOME?.trim();
  if (!configured) return path.join(os.homedir(), ".nuncioade");
...
export function discoverExternalMcpRuntime(baseDir: string): {
  ...
  if (candidates.length === 0) {
    throw new ExternalMcpBridgeError(
      `No running NuncioADE instance was found under ${baseDir}. Start NuncioADE first or pass --home-dir for the intended instance.`,
    );
  }
  if (candidates.length > 1) {
    throw new ExternalMcpBridgeError(
      `Multiple running NuncioADE instances were found under ${baseDir}: ${candidates.map((candidate) => candidate.state.origin).join(", ")}. Stop one instance or pass a distinct --home-dir.`,
    );
  }
  return candidates[0]!;
}
...
export function writeExternalMcpClientCredential(
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (existing.credential !== paired.credential) {
      throw new ExternalMcpBridgeError(
        `Integration ${paired.integrationId} already has a different stored credential. Revoke it in NuncioADE before replacing the local secret.`,
      );
    }
    return filePath;
  }
...
export function readExternalMcpClientCredential(
  ...
  if (candidates.length === 0 || !fs.existsSync(candidates[0]!)) {
    throw new ExternalMcpBridgeError(
      integrationId
        ? `No paired external MCP credential was found for integration ${integrationId}. Run its pairing command from NuncioADE Settings.`
        : `No paired external MCP credential was found under ${directory}. Create an integration in NuncioADE Settings, then run its pairing command.`,
    );
  }
  if (candidates.length > 1) {
...
export async function readExternalMcpResponseText(
  ...
  const timeout = new Promise<never>((_resolve, reject) => {
    timer = setTimeout(() => {
      ...
      reject(new ExternalMcpBridgeError(`NuncioADE response body stalled for ${timeoutMs} ms.`));
    }, timeoutMs);
  });
  try {
...
export async function verifyExternalMcpRuntime(
  ...
  if (
    ...
    throw new ExternalMcpBridgeError(
      "The loopback endpoint did not prove it is the NuncioADE process named by the private runtime-state file.",
    );
  }
}
...
export function requestTimeoutForBody(body: string): number {
  try {
    ...
    for (const value of messages) {
      ...
      if (request.params?.name === "nuncioade_create_task") {
        serverWorkMs += EXTERNAL_MCP_CREATE_TIMEOUT_MS;
        continue;
      }
      if (request.params?.name !== "nuncioade_wait_for_task") continue;
...
async function fetchWithRestartRecovery(input: {
  ...
  throw new ExternalMcpBridgeError(
    "Could not authenticate and reconnect to NuncioADE. Ensure exactly one intended instance is running.",
    { cause: lastCause },
  );
}
...
export async function pairExternalMcpClient(input: {
  ...
  if (!response) {
    throw new ExternalMcpBridgeError(
      "Could not reconnect to NuncioADE to complete pairing. The private pending credential was preserved for a safe retry.",
      { cause: lastCause },
    );
  }
  ...
  if (!response.ok || !body || !("credential" in body)) {
    throw new ExternalMcpBridgeError(
      (body && "error" in body && body.error) ||
        `NuncioADE rejected external MCP pairing with HTTP ${response.status}.`,
    );
  }
  if (body.credential !== pending.value.credential) {
    throw new ExternalMcpBridgeError(
      "NuncioADE returned a different pairing credential; refusing it.",
    );
  }
...
export async function serveExternalMcpStdio(input: {
  ...
  const handleMessage = async (
    ...
    try {
      ...
      await emit(stderr, `[nuncioade mcp] ${message}\n`);
    ...
    if (response.status === 401) {
      const message =
        "NuncioADE rejected the stored external MCP credential because it was revoked, expired, or replaced. Pair the integration again from Settings.";
      await emit(stderr, `[nuncioade mcp] ${message}\n`);
    ...
    if (!response.ok) {
      ...
      const message = `NuncioADE external MCP request failed with HTTP ${response.status}.`;
      await emit(stderr, `[nuncioade mcp] ${message}\n`);
      return localErrorResponse(line, -32603, message);
    }
  ...
  const handleLine = async (line: string): Promise<void> => {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        const fallback = localErrorResponse(entry.line, -32603, "Invalid NuncioADE response");
