...
// Purpose: Build a ZIP archive that exports a single NuncioADE thread so a user
...
// The export-eligibility guard lives in @nuncio/shared/threadExport so the
// web composer and the HTTP route share one predicate.

import zlib from "node:zlib";

import type { OrchestrationMessage, OrchestrationThread } from "@nuncio/contracts";
...
export function threadArchiveFileName(input: {
  ...
  return `nuncioade-thread-${slugifyTitle(input.title)}-${dateBucket}.zip`;
}
