...
function projectCreateCommand(
  ...
  return {
    ...
    kind: "chat",
    title: "Chat",
    workspaceRoot: "/Users/tester/Documents/NuncioADE/2026-06-11/chat",
...
describe("makeDispatchCommandNormalizer", () => {
  it("returns a deferred prepare effect instead of scaffolding during normalization", async () => {
    ...
    expect(preparedRoots).toEqual(["/Users/tester/Documents/NuncioADE/2026-06-11/chat"]);
  });
  ...
  it("does not prepare ordinary projects or the chat workspace root itself", async () => {
    ...
    const first = await Effect.runPromise(
      normalizer({
        command: projectCreateCommand({
          kind: "project",
          workspaceRoot: "/Users/tester/Documents/NuncioADE/2026-06-11/app",
        }),
      }),
    );
    ...
    const second = await Effect.runPromise(
      normalizer({
        command: projectCreateCommand({
          workspaceRoot: "/Users/tester/Documents/NuncioADE",
        }),
      }),
    );
  ...
  it("prepares the Studio workspace root itself", async () => {
    ...
    expect(preparedRoots).toEqual(["/Users/tester/Documents/NuncioADE/Studio"]);
  });
  ...
  it("prepares nested Studio workspace roots but not ordinary projects under Studio", async () => {
    ...
    const first = await Effect.runPromise(
      normalizer({
        command: projectCreateCommand({
          kind: "studio",
          workspaceRoot: "/Users/tester/Documents/NuncioADE/Studio/Outbox",
        }),
      }),
    );
    await runPrepareWorkspaceRoot(first);
    const second = await Effect.runPromise(
      normalizer({
        command: projectCreateCommand({
          kind: "project",
          workspaceRoot: "/Users/tester/Documents/NuncioADE/Studio/SomeProject",
        }),
      }),
    );
    await runPrepareWorkspaceRoot(second);

    expect(preparedRoots).toEqual(["/Users/tester/Documents/NuncioADE/Studio/Outbox"]);
  });

  it("defers binary attachment authority to the transactional managed ledger", async () => {
    const attachmentsDir = fs.mkdtempSync(path.join(os.tmpdir(), "nuncioade-dispatch-normalize-"));
