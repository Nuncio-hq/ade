...
export function formatMissingCodexWorkingDirectoryError(cwd: string): string {
  return `Project working directory no longer exists: ${cwd}. Relocate or reconnect the project in NuncioADE.`;
}
...
export function assertCodexWorkingDirectoryExists(cwd: string): void {
  try {
    ...
    if (!stats.isDirectory()) {
      throw new Error(
        `Project working directory is not a directory: ${cwd}. Relocate or reconnect the project in NuncioADE.`,
      );
    }
  } catch (error) {
