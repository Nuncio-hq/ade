...
import type { WsCompatibilityError } from "@nuncio/contracts";

export type WsTransportState = "connecting" | "open" | "closed" | "incompatible" | "disposed";

export const NUNCIO_WS_TRANSPORT_STATE_EVENT = "nuncioade:ws-transport-state";
export const NUNCIO_WS_COMPATIBILITY_ISSUE_EVENT = "nuncioade:ws-compatibility-issue";
...
export function emitWsTransportState(state: WsTransportState): void {
  ...
  window.dispatchEvent(
    new CustomEvent<WsTransportStateEventDetail>(NUNCIO_WS_TRANSPORT_STATE_EVENT, {
      detail: { state },
    }),
  );
}
...
export function addWsTransportStateListener(
  ...
  window.addEventListener(NUNCIO_WS_TRANSPORT_STATE_EVENT, handleStateChange);
  return () => {
    window.removeEventListener(NUNCIO_WS_TRANSPORT_STATE_EVENT, handleStateChange);
  };
}
...
export function emitWsCompatibilityIssue(issue: WsCompatibilityError | null): void {
  ...
  window.dispatchEvent(
    new CustomEvent<WsCompatibilityIssueEventDetail>(NUNCIO_WS_COMPATIBILITY_ISSUE_EVENT, {
      detail: { issue },
    }),
  );
}
...
export function addWsCompatibilityIssueListener(
  ...
  window.addEventListener(NUNCIO_WS_COMPATIBILITY_ISSUE_EVENT, handleIssue);
  return () => {
    window.removeEventListener(NUNCIO_WS_COMPATIBILITY_ISSUE_EVENT, handleIssue);
  };
}
