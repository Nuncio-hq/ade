import type { GitBranch } from "@nuncio/contracts";
...
describe("resolveDraftEnvModeAfterBranchChange", () => {
  it("switches to local mode when returning from an existing worktree to the main worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        ...
        currentWorktreePath: "/repo/.nuncioade/worktrees/feature-a",
  ...
  it("uses worktree mode when selecting a branch already attached to a worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.nuncioade/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("keeps legacy .nuncioade worktree paths working for migrated threads", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.nuncioade/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("syncs stale local thread metadata to the concrete git checkout", () => {
    expect(
      shouldSyncLocalThreadBranch({
        envMode: "local",
        activeWorktreePath: null,
        activeThreadBranch: "nuncioade/pi",
        currentGitBranch: "main",
  ...
  it("preserves associated worktree metadata during local branch-only syncs", () => {
    expect(
      resolveAssociatedWorktreeMetadataAfterWorkspacePatch({
        branch: "main",
        worktreePath: null,
        existingAssociatedWorktreePath: "/repo/.worktrees/nuncioade-pi",
        existingAssociatedWorktreeBranch: "nuncioade/pi",
        existingAssociatedWorktreeRef: "nuncioade/pi",
      }),
    ).toEqual({
      associatedWorktreePath: "/repo/.worktrees/nuncioade-pi",
      associatedWorktreeBranch: "nuncioade/pi",
      associatedWorktreeRef: "nuncioade/pi",
    });
  });
...
describe("resolveBranchSelectionTarget", () => {
  it("reuses an existing secondary worktree for the selected branch", () => {
    expect(
      resolveBranchSelectionTarget({
        ...
        activeWorktreePath: "/repo/.nuncioade/worktrees/feature-a",
        branch: {
          isDefault: false,
          worktreePath: "/repo/.nuncioade/worktrees/feature-b",
        },
      }),
    ).toEqual({
      checkoutCwd: "/repo/.nuncioade/worktrees/feature-b",
      nextWorktreePath: "/repo/.nuncioade/worktrees/feature-b",
  ...
  it("keeps checkout in the current worktree for non-default branches", () => {
    expect(
      ...
      checkoutCwd: "/repo/.nuncioade/worktrees/feature-a",
      nextWorktreePath: "/repo/.nuncioade/worktrees/feature-a",
      reuseExistingWorktree: false,
    });
  });
