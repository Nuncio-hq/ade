...
import { appSnapShortcutLabels } from "@nuncio/shared/appSnapShortcut";
...
function appSnapStatusText(state: DesktopAppSnapState | null): string {
  if (!state) return "Available in the NuncioADE desktop app";
...
export function NotificationsSettingsPanel({
  ...
  async function sendTestNotification() {
    ...
    const notification = new Notification(title, { body, tag: "nuncioade:test-notification" });
  ...
  async function setAppSnapEnabled(nextEnabled: boolean) {
    ...
    if (!bridge) {
      toastManager.add({
        ...
        description: "AppSnap requires the NuncioADE desktop app on macOS.",
      });
      return;
    }
    ...
    <div className="space-y-6">
          ...
          {!supported ? (
            <p className={cn(SETTINGS_CARD_ROW_DESCRIPTION_CLASS_NAME, "pt-0.5")}>
              {appSnapState
                ...
                : "AppSnap requires the NuncioADE desktop app on macOS."}
            </p>
          ) : null}
        </div>
      </SettingsCard>
      ...
      <SettingsSection title="Capture">
        <SettingsRow
          ...
          description="Run the capture listener in the background while NuncioADE is open."
        ...
        <SettingsRow
          title="Shortcut"
          description="Choose exactly two keys: one modifier and one other key. NuncioADE checks its own bindings and asks macOS whether another app already owns the shortcut before saving it."
          control={
        ...
        <SettingsRow
          title="Destination"
          description="Snaps join the task you interacted with in the last minute, and consecutive snaps stay together. Otherwise NuncioADE opens a fresh task with the capture attached."
      ...
      {supported ? (
        <SettingsSection title="macOS permissions">
          <SettingsRow
            ...
            description="Lets NuncioADE notice the double-Option chord while another app owns the keyboard. Nothing you type is recorded."
            control={<AppSnapPermissionBadge permission={appSnapState.inputMonitoringPermission} />}
          />
          <SettingsRow
            title="Screen Recording"
            description="Lets NuncioADE capture an image of the frontmost window. Only the single window you snap is captured, only at the moment you press the chord."
            control={<AppSnapPermissionBadge permission={appSnapState.screenRecordingPermission} />}
          />
          <SettingsRow
            title="Permission status"
            description="Grant both permissions to NuncioADE under System Settings → Privacy & Security, then recheck here. macOS may require relaunching the app after a change."
            control={
              <Button
                type="button"
                size="xs"
