    ...
    <div className="space-y-6">
      {!setup ? (
        <SettingsSection title="Connect a coding agent">
          <SettingsRow
            title="Name"
            description="How this connection appears in NuncioADE. Works with Codex, Claude, and any other MCP-capable agent."
            control={
              <Input
      ...
      {setup && setupIntegration && setupPrompt && manualConfiguration && examplePrompt ? (
        <SettingsSection title={`Connect ${setupIntegration.name}`}>
          <SettingsRow
            ...
            description={
              revoked
                ? "This connection has been revoked and can no longer access NuncioADE."
                : integrationExpired
                  ? "This connection has expired and can no longer access NuncioADE."
                  : connected
                    ? "NuncioADE received a request from this agent. Setup is complete."
                    : paired
                      ? "The private credential is stored locally. If the agent has not registered NuncioADE yet, give it the setup prompt below."
                      : pairingExpired
          ...
          <SettingsRow
            ...
            description="Copy the prompt and paste it into the agent you want to connect (Codex, Claude Code, or any MCP-capable app). The agent pairs this computer, registers NuncioADE in its own configuration, and verifies the connection by itself."
            status={
              paired
          ...
          <SettingsRow
            ...
            status={
              connected
                ? "Connection verified by NuncioADE."
                : "NuncioADE will show Connected after the agent makes its first request."
            }
            control={
              <Button
                size="xs"
      ...
      <SettingsSection title="Connected agents">
        {integrationsQuery.isLoading ? (
          ...
          <SettingsListRow
            ...
            description="Connect Codex, Claude, or another local MCP agent to create and follow NuncioADE tasks."
          />
        )}
      </SettingsSection>
    </div>
