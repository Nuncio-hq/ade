...
// Purpose: Routes native macOS AppSnaps into the correct NuncioADE composer draft.
    ...
    async (capture: DesktopAppSnapCapture) => {
      ...
      if (resolvedTarget.kind === "existing") {
        ...
        if (result.threadId) {
          ...
          const focused = focusedTargetRef.current;
          if (!focused) throw new Error("NuncioADE could not create a task for this AppSnap.");
          target = focused;
          openChatThreadPage(target.threadId);
        }
      }
      ...
      if (!image) throw new Error(error ?? "NuncioADE could not attach the captured AppSnap.");
      ...
      toastManager.add({
        ...
        description:
          persistenceResult === "unverified"
            ? "The capture is attached, but NuncioADE could not verify its draft metadata. If it is missing after a reload, NuncioADE will attach it again."
