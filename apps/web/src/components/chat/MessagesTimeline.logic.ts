...
import { type MessageId, type TurnId } from "@nuncio/contracts";
...
function workLogNuncioADEThreadCreationsEqual(
  a: WorkLogEntry["nuncioadeThreadCreation"],
  b: WorkLogEntry["nuncioadeThreadCreation"],
) {
  if (a === b) return true;
  if (!a || !b) return false;
  if (
...
function workLogEntryContentEqual(a: WorkLogEntry, b: WorkLogEntry): boolean {
  return (
    ...
    workLogNuncioADEThreadCreationsEqual(a.nuncioadeThreadCreation, b.nuncioadeThreadCreation) &&
