...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.nuncioade/worktrees/nuncioade-mvp/nuncioade-4e609bb8",
    );
    expect(result).toBe("nuncioade-4e609bb8");
  });

  it("keeps legacy .nuncioade worktree paths readable after migration", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.nuncioade/worktrees/nuncioade-mvp/nuncioade-legacy123",
    );
    expect(result).toBe("nuncioade-legacy123");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.nuncioade\\worktrees\\nuncioade-mvp\\nuncioade-4e609bb8",
    );
    expect(result).toBe("nuncioade-4e609bb8");
  });

  it("uses the final segment even when outside ~/.nuncioade/worktrees", () => {
