...
import { isSupportedLocalPreviewFilePath } from "@nuncio/shared/localPreviewFiles";
import {
  isLocalAbsolutePath,
...
const NUNCIO_PUBLIC_ASSET_PATH_PREFIXES = [
  "/central-icons-reversed/",
  "/central-icons-fill/",
] as const;
const NUNCIO_WEB_PUBLIC_WORKSPACE_DIR = "apps/web/public";

function resolveNuncioADEPublicAssetOpenTarget(path: string, workspaceRoot: string | null) {
  if (!workspaceRoot) {
    return null;
  }
  const normalizedPath = path.replace(/\\/g, "/");
  if (!NUNCIO_PUBLIC_ASSET_PATH_PREFIXES.some((prefix) => normalizedPath.startsWith(prefix))) {
    return null;
  }
  const relativePath = `${NUNCIO_WEB_PUBLIC_WORKSPACE_DIR}${normalizedPath}`;
...
export function resolveWorkspaceFileOpenTarget(
  ...
  return resolveNuncioADEPublicAssetOpenTarget(withoutPosition, workspaceRoot);
}
