...
describe("isPrefixedToolArgumentSummary", () => {
  it("recognizes `ToolName: {json}` and `ToolName: [json]` details", () => {
    expect(
      isPrefixedToolArgumentSummary('mcp__nuncioade__nuncioade_read_thread: {"threadId":"c357"}'),
    ).toBe(true);
...
describe("toolArgumentSummaryToolName", () => {
  it("extracts the identifier prefix whether or not JSON args follow", () => {
    ...
    expect(toolArgumentSummaryToolName('mcp__nuncioade__nuncioade_read_thread: {"threadId":"c3"}')).toBe(
      "mcp__nuncioade__nuncioade_read_thread",
    );
  });
