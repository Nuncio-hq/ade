...
import { type ProjectId, type ThreadId, type TurnId } from "@nuncio/contracts";
...
// Keep the v1 suffix stable while using the NuncioADE namespace; legacy
// `nuncioade:*` and `nuncioade:*` keys are copied over by
// `storageKeyMigration` before this store hydrates, so older payloads still
// flow through the v1 -> v2 schema migration below.
const SPLIT_VIEW_STORAGE_KEY = "nuncioade:split-view-state:v1";
