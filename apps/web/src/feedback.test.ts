...
const DIAGNOSTICS: FeedbackDiagnostics = {
  ...
  userAgent: "NuncioADE test agent",
  platform: "MacIntel",
  language: "en-US",
  viewport: "1440x900",
};
...
describe("formatFeedbackSummary", () => {
  it("opens in the reporter's voice and lists the diagnostics a maintainer needs", () => {
    ...
    expect(summary).toBe(
      [
        "I ran into a bug in NuncioADE 0.5.1, using codex with gpt-5.6-sol.",
        "",
        "Report type: Bug",
        "App version: 0.5.1",
        "Provider: codex",
        ...
        "Language: en-US",
        "User agent: NuncioADE test agent",
  ...
  it("falls back to a neutral opening and omits fields the session never set", () => {
    ...
    expect(summary).toContain(
      "I have some feedback in NuncioADE 0.5.1, using codex with gpt-5.6-sol.",
    );
  ...
  it.each(FEEDBACK_CATEGORIES)(
    ...
    ({ value, label, lead }) => {
      ...
      expect(summary.startsWith(`${lead} in NuncioADE 0.5.1`)).toBe(true);
  ...
  it("describes feedback sent outside an active chat without inventing provider context", () => {
    ...
    expect(summary).toContain("I have some feedback in NuncioADE 0.5.1 outside an active chat.");
