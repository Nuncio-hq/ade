...
NuncioADE can expose a small, user-approved MCP tool surface to another local app such as Codex or
Claude. This integration is separate from the internal `/mcp` endpoint injected into provider
sessions.

## Guided setup

1. Start NuncioADE and open **Settings → Integrations**.
...
4. For Codex, Claude Code, or another agentic MCP client, copy the generated setup prompt into the
   ...
   stdio configuration, and verifies the connection with `nuncioade_overview`. For Claude Desktop or a
   client that cannot run the setup prompt, complete pairing in NuncioADE and use the copy-ready JSON
   configuration instead. NuncioADE uses the exact executable and data directory of the running
   installation; no global `nuncioade` command, project ID, model slug, request ID, or credential
   handling is required from the user.
5. NuncioADE moves from **Waiting for pairing** to **Paired** after the local credential exchange, then
...
```sh
/absolute/path/to/runtime /absolute/path/to/nuncioade-server mcp serve --integration mcp_int_REDACTED --home-dir "$HOME/.nuncioade"
```

The Codex copy action generates:

```sh
codex mcp add nuncioade [--env ELECTRON_RUN_AS_NODE=1] -- /absolute/runtime /absolute/server mcp serve --integration mcp_int_REDACTED --home-dir "$HOME/.nuncioade"
```

The Claude Code copy action generates a user-scoped configuration:

```sh
claude mcp add --scope user nuncioade [-e ELECTRON_RUN_AS_NODE=1] -- /absolute/runtime /absolute/server mcp serve --integration mcp_int_REDACTED --home-dir "$HOME/.nuncioade"
```
...
args = ["/absolute/path/to/nuncioade-server", "mcp", "serve", "--integration", "mcp_int_REDACTED", "--home-dir", "/absolute/path/to/nuncioade-data"]
...
```json
{
  "mcpServers": {
    "nuncioade": {
      "command": "/absolute/path/to/runtime",
      "args": [
        "/absolute/path/to/nuncioade-server",
        "mcp",
        "serve",
        "--integration",
        "mcp_int_REDACTED",
        "--home-dir",
        "/absolute/path/to/nuncioade-data"
      ],
...
NuncioADE always includes its actual data directory in generated setup, so multiple installations do not
...
`<NuncioADE home>/mcp/credentials/<integration-id>.json`. NuncioADE creates its parent directory with mode `0700`
and the file with mode `0600` on POSIX systems. On Windows the file remains under the current user
profile, but Windows does not provide the same POSIX mode guarantee; protect the account and NuncioADE
...
- `nuncioade_overview` — orient in one call with allowed projects, paths and activity, provider
  availability, granted scopes, safe defaults, limits, and suggested next steps.
- `nuncioade_capabilities` — provider/model construction and safety limits for an allowed project.
- `nuncioade_list_allowed_projects` — only projects selected by the user.
- `nuncioade_create_task` — one task per stable `requestId`.
- `nuncioade_wait_for_task` — wait for an authorized task without changing it.
- `nuncioade_read_task` — read tasks created by the integration. Reading other tasks requires the
...
- `/mcp/external` is available only while NuncioADE itself is loopback-only. Configuring remote or
  published access disables the external endpoint instead of exposing it remotely.
- External credentials have the fixed `nuncioade.external-mcp` audience. They are opaque, expiring,
...
The stdio bridge re-reads NuncioADE's private runtime-state file on each request and requires the
