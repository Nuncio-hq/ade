...
The goal was to investigate why NuncioADE could appear to consume many gigabytes of memory with only one visible chat. The diagnosis found stale provider subprocesses, especially orphaned `kilo serve` processes, and the code change makes Electron wait for backend shutdown instead of exiting immediately after sending `SIGTERM`. Desktop package tests pass.
...
Think of NuncioADE like a teacher leaving a classroom. The teacher also has helpers doing work in side rooms. Before this change, the teacher shouted "pack up" and walked out right away, so some helpers could keep sitting in those side rooms all night.
