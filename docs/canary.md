...
NuncioADE Canary is a frozen local build of a chosen remote Git ref. It is intentionally separate
from both NuncioADE Stable and the HMR development process.

## Isolation

- App name: `NuncioADE Canary`
- Bundle ID: `com.emanueledipietro.nuncioade.canary`
- Desktop origin: `nuncioade-canary://app`
- NuncioADE data: `~/.nuncioade-canary`
- Electron profile: `nuncioade-canary`
- Managed source: `~/.cache/nuncioade-canary/source`
- Runtime log: `~/.nuncioade-canary/canary.log`
...
```bash
bun run canary:setup -- --ref codex/nuncioade-canary
```
...
```bash
NUNCIO_CANARY_HOME=/path/to/data \
NUNCIO_CANARY_SOURCE=/path/to/source \
bun run canary:update
```
...
```bash
env -u NUNCIO_AUTH_TOKEN \
  NUNCIO_DEV_INSTANCE=my-feature \
  bun run dev -- --home-dir ./.nuncioade/dev-my-feature
```
