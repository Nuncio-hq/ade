...
- Publishes default `latest*.yml` metadata plus byte-identical `nuncioade*.yml` aliases on every stable release so existing packaged binaries keep working.
...
- Publishes prerelease installers only on their versioned GitHub prerelease; prereleases never replace the stable `nuncioade` update manifests.
- Publishes the CLI package (`apps/server`, npm package `@nuncio/cli`) with OIDC trusted publishing.
...
- Runtime channel: `nuncioade`. Stable 0.5.x releases publish both `latest` and `nuncioade` metadata; the 0.4.x compatibility release remains available for historical migration.
- Repository slug source:
  - `NUNCIO_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Required NuncioADE release assets for updater:
  - platform installers (`.exe`, `.dmg`, `.AppImage`, plus macOS `.zip` for Squirrel.Mac update payloads)
  - `nuncioade-mac.yml`, `nuncioade.yml`, and `nuncioade-linux.yml` metadata
  - every stable release includes both `nuncioade-mac.yml`, `nuncioade.yml`, `nuncioade-linux.yml` and `latest-mac.yml`, `latest.yml`, `latest-linux.yml`
  - `*.blockmap` files, except the macOS update `.zip.blockmap` removed after zip repack
- Enforced upgrade path:
  - Stable clean NuncioADE releases are created with `make_latest=true` and carry both six-manifest filenames in the versioned release.
  ...
  - Clean-release publication fails closed if either the default Latest manifests or the dedicated `nuncioade` aliases are missing.
- Production desktop builds omit web/server/desktop source maps by default to keep update payloads small. Set `NUNCIO_WEB_SOURCEMAP=1`, `NUNCIO_SERVER_SOURCEMAP=1`, or `NUNCIO_DESKTOP_SOURCEMAP=1` only for a diagnostic release that needs them.
- macOS metadata note:
  - The build initially emits `latest-mac.yml` for both Intel and Apple Silicon.
  - The workflow merges the per-arch macOS metadata, then keeps the merged manifest as `latest-mac.yml` and copies it to `nuncioade-mac.yml` for stable releases.
...
1. Confirm the npm account controls the `@nuncioade` scope and can publish `@nuncio/cli`.
...
- The published release title should read `NuncioADE vX.Y.Z`.
...
- Optional jobs stay disabled unless repository variables enable them:
  - `NUNCIO_PUBLISH_CLI=1`
  - `NUNCIO_FINALIZE_RELEASE=1`
...
8. Confirm the new versioned release is not GitHub Latest and the pinned compatibility release contains the new payloads plus all three `nuncioade` manifests.
