...
Imagine Droid gives NuncioADE a menu for GPT-5.6 with choices like Low, Medium, High, and Extra High. NuncioADE received that menu but forgot that Droid was allowed to use it, so it showed nothing. Now Droid is on the allowed list. NuncioADE displays the choices and remembers the one you picked when it sends your message.
