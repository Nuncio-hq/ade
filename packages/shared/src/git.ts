 ...
 */
export const WORKTREE_BRANCH_PREFIX = "nuncioade";
...
const NUNCIO_BRANCH_FALLBACK = "update";
...
export function buildNuncioADEBranchName(preferredBranch?: string | null): string {
  const preferred = preferredBranch?.trim() ?? "";
  ...
  return `${WORKTREE_BRANCH_PREFIX}/${sanitizeBranchFragment(
    normalizedExisting || NUNCIO_BRANCH_FALLBACK,
  )}`;
}

export function resolveUniqueNuncioADEBranchName(
  existingBranchNames: readonly string[],
  preferredBranch?: string | null,
): string {
  const resolvedBase = buildNuncioADEBranchName(preferredBranch);
  return resolveUniqueBranchName(existingBranchNames, resolvedBase);
}
