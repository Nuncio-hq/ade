...
describe("desktopIdentity", () => {
  it("uses the exact canonical production and development bundle IDs", () => {
    expect(NUNCIO_PRODUCTION_BUNDLE_ID).toBe("com.emanueledipietro.nuncioade");
    expect(NUNCIO_DEVELOPMENT_BUNDLE_ID).toBe("com.emanueledipietro.nuncioade.dev");
  ...
  it("gives Canary a fully separate desktop identity and storage profile", () => {
    expect(NUNCIO_CANARY_BUNDLE_ID).toBe("com.emanueledipietro.nuncioade.canary");
  ...
  it("selects Canary explicitly without changing normal dev and production defaults", () => {
    ...
    expect(resolveNuncioADEDesktopFlavor({ isDevelopment: false, requestedFlavor: " canary " })).toBe(
      "canary",
    );
