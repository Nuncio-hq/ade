...
import {
  NuncioADECapabilitiesResult,
  NuncioADECreateThreadsInput,
  NuncioADECreateThreadsResult,
  NuncioADEGatewayErrorResult,
  NuncioADEWaitForThreadsInput,
  NuncioADEWaitForThreadsResult,
} from "./agentGateway";

const decodeCreate = Schema.decodeUnknownSync(NuncioADECreateThreadsInput);
const decodeWait = Schema.decodeUnknownSync(NuncioADEWaitForThreadsInput);
...
describe("agent gateway contracts", () => {
  ...
  it("decodes typed capability, creation, wait, and error results", () => {
    assert.doesNotThrow(() =>
      Schema.decodeUnknownSync(NuncioADECapabilitiesResult)({
        targetConstruction: {
          codex: {
    ...
    assert.doesNotThrow(() =>
      Schema.decodeUnknownSync(NuncioADECreateThreadsResult)({
    ...
    assert.doesNotThrow(() =>
      Schema.decodeUnknownSync(NuncioADEWaitForThreadsResult)({
    ...
    assert.doesNotThrow(() =>
      Schema.decodeUnknownSync(NuncioADEGatewayErrorResult)({
