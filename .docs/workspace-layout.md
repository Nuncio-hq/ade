...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `nuncioade` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@nuncio/shared/git`, `@nuncio/shared/DrainableWorker`) — no barrel index.
