...
- Dev commands default `NUNCIO_HOME` to `~/.nuncioade`, which keeps dev state under `~/.nuncioade/dev`.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --home-dir ~/.nuncioade-2`
...
- Desktop production windows load the bundled UI from `nuncioade://app/index.html` (not a `127.0.0.1` document URL).
- Desktop packaging includes `apps/server/dist` (the `nuncioade` backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
Set `NUNCIO_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `NUNCIO_DEV_INSTANCE`)
- Example: `NUNCIO_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `NUNCIO_PORT_OFFSET` to a numeric offset.
