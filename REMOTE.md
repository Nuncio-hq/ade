...
Use this when you want to open NuncioADE from another device (phone, tablet, another laptop).

## CLI ↔ Env option map

The NuncioADE CLI accepts the following configuration options, available either as CLI flags or environment variables:

| CLI flag                | Env var               | Notes                              |
| ----------------------- | --------------------- | ---------------------------------- |
| `--mode <web\|desktop>` | `NUNCIO_MODE`         | Runtime mode.                      |
| `--port <number>`       | `NUNCIO_PORT`         | HTTP/WebSocket port.               |
| `--host <address>`      | `NUNCIO_HOST`         | Bind interface/address.            |
| `--home-dir <path>`     | `NUNCIO_HOME`         | Base directory.                    |
| `--dev-url <url>`       | `VITE_DEV_SERVER_URL` | Dev web URL redirect/proxy target. |
| `--no-browser`          | `NUNCIO_NO_BROWSER`   | Disable auto-open browser.         |
| `--auth-token <token>`  | `NUNCIO_AUTH_TOKEN`   | WebSocket auth token.              |
