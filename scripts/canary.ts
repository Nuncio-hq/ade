// FILE: canary.ts
// Purpose: Maintains and launches an isolated, frozen NuncioADE Canary checkout.
...
export function resolveCanaryPaths(
  ...
  const home = Path.resolve(
    env.NUNCIO_CANARY_HOME?.trim() || Path.join(homeDirectory, ".nuncioade-canary"),
  );
  const cacheBase = env.XDG_CACHE_HOME?.trim() || Path.join(homeDirectory, ".cache");
  const source = Path.resolve(
    env.NUNCIO_CANARY_SOURCE?.trim() || Path.join(cacheBase, "nuncioade-canary", "source"),
  );
  return {
    home,
    source,
...
function assertManagedSourceIsClean(paths: CanaryPaths): void {
  ...
  if (status.length > 0) {
    throw new Error(
      `NuncioADE Canary's managed source has tracked local changes. Refusing to overwrite ${paths.source}.`,
    );
  }
}
...
function startCanary(paths: CanaryPaths): void {
  ...
  if (existingPid !== null && isRunning(existingPid)) {
    console.log(`NuncioADE Canary is already running (pid ${String(existingPid)}).`);
    return;
  }
  ...
  if (
    ...
    throw new Error("NuncioADE Canary is not built. Run `bun run canary:setup` first.");
  }
  FS.mkdirSync(paths.home, { recursive: true });
  const env = { ...process.env };
  delete env.VITE_DEV_SERVER_URL;
  delete env.ELECTRON_RENDERER_PORT;
  delete env.NUNCIO_AUTH_TOKEN;
  Object.assign(env, {
    NUNCIO_DESKTOP_FLAVOR: "canary",
    NUNCIO_DISABLE_AUTO_UPDATE: "1",
    NUNCIO_HOME: paths.home,
    NUNCIO_COMMIT_HASH: commit,
  });
  ...
  try {
    ...
    if (child.pid === undefined) {
      throw new Error("NuncioADE Canary failed to return a process id.");
    }
    child.unref();
    FS.writeFileSync(paths.pid, `${String(child.pid)}\n`, { mode: 0o600 });
    console.log(`Started NuncioADE Canary at ${commit.slice(0, 12)} (pid ${String(child.pid)}).`);
...
function rollbackCanary(paths: CanaryPaths): void {
  ...
  if (state?.previousCommit === null || state?.previousCommit === undefined) {
    throw new Error("NuncioADE Canary has no previous successful commit to restore.");
  }
  assertManagedSourceIsClean(paths);
  stopCanary(paths);
...
function printStatus(paths: CanaryPaths): void {
  ...
  console.log(`NuncioADE Canary: ${running ? `running (pid ${String(pid)})` : "stopped"}`);
