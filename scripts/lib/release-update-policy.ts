...
// dedicated `nuncioade` channel aliases.
...
export function resolveReleaseUpdatePolicy(
  ...
  if (normalizedConfig.lane === "clean" && compareCoreVersions(requested.core, bridge.core) <= 0) {
    throw new Error(
      `NuncioADE releases must be newer than the compatibility release v${normalizedConfig.bridgeVersion}.`,
    );
  }
...
export function prepareReleaseUpdateManifests(
  ...
  // request the dedicated `nuncioade` channel. Keep both filenames in the same
