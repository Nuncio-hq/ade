...
function createAssets(): string {
  const root = mkdtempSync(join(tmpdir(), "nuncioade-artifact-provenance-test-"));
  temporaryRoots.push(root);
  writeFileSync(join(root, "NuncioADE-1.2.3-x64.AppImage"), "app-image-bytes");
  writeFileSync(join(root, "latest-linux.yml"), "version: 1.2.3\n");
  return root;
}

function createWindowsAssets(): string {
  const root = mkdtempSync(join(tmpdir(), "nuncioade-windows-provenance-test-"));
  temporaryRoots.push(root);
  writeFileSync(join(root, "NuncioADE-1.2.3-x64.exe"), "unsigned-windows-bytes");
...
describe("release artifact provenance", () => {
  it("hashes the exact collected Linux assets into a deterministic manifest", async () => {
    ...
    expect(result.manifest.artifacts.map((artifact) => artifact.fileName)).toEqual([
      "latest-linux.yml",
      "NuncioADE-1.2.3-x64.AppImage",
    ]);
    expect(
      result.manifest.artifacts.find(
        (artifact) => artifact.fileName === "NuncioADE-1.2.3-x64.AppImage",
      )?.sha256,
