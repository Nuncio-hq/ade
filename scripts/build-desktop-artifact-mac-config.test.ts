...
describe("createDesktopPlatformBuildConfig", () => {
  it("adds explicit microphone entitlements to macOS builds", () => {
    ...
    assert.equal(MAC_APPSNAP_HELPER_BUNDLE_PATH, "Contents/Helpers/nuncioade-appsnap-helper");
    assert.deepStrictEqual(mac.binaries, ["Contents/Helpers/nuncioade-appsnap-helper"]);
    assert.equal(mac.x64ArchFiles, "Contents/Helpers/nuncioade-appsnap-helper");
    assert.equal(
      MAC_APPSNAP_HELPER_STAGE_PATH,
    ...
    assert.deepStrictEqual(config.extraFiles, [
      {
        from: "apps/desktop/native/appsnap/build/nuncioade-appsnap-helper",
  ...
  it("leaves non-macOS platform configs unchanged", () => {
    ...
    const win = createDesktopPlatformBuildConfig({
      platform: "win",
      target: "nsis",
      windowsAzureSignOptions: { publisherName: "NuncioADE" },
    });
    ...
    assert.deepStrictEqual(win.win, {
      target: ["nsis"],
      icon: "icon.ico",
      publisherName: "NuncioADE",
      azureSignOptions: { publisherName: "NuncioADE" },
    });
  });
