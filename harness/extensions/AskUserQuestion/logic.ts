...
export function decideOptionListEnterAction(
  currentQuestionIndex: number,
  totalQuestions: number,
): OptionListEnterAction {
...
export function setFocusedIndex(
  ...
  return normalizeState(question, {
    ...state,
    focusedIndex: clampIndex(index, getOptionCount(question)),
  });
}

export function toggleFocusedOption(
  question: AskUserQuestionItem,
  state: SelectionState,
): SelectionState {
  const current = normalizeState(question, state);
...
export function setFocusedChoiceNote(
  question: AskUserQuestionItem,
  state: SelectionState,
  note: string,
): SelectionState {
...
export function getSelectedChoiceNotes(
  question: AskUserQuestionItem,
  state: SelectionState,
): Record<string, string> {
  const current = normalizeState(question, state);
  const selected = new Set(
    current.selectedIndexes.filter((index) => !isCustomOption(question, index)),
  );
...
export function cycleFocusMode(
  ...
  if (
    currentFocus.mode === "options" &&
    currentState.focusedIndex >= 0 &&
    currentState.focusedIndex < question.options.length
  ) {
