...
const QuestionSchema = Type.Object({
  header: Type.String({
    description: "Short header used as the key in the returned result mapping.",
  }),
  question: Type.String({ description: "Question text to show to the user." }),
  multiSelect: Type.Boolean({
    description:
      "If true, user may toggle multiple options. If false, exactly one option is selected.",
  }),
...
function formatDetailsForContent(details: AskUserQuestionStructuredResult): string {
  if (details.unavailable) {
    return JSON.stringify(
      { unavailable: true, reason: details.reason, answers: details.answers },
      null,
      2,
    );
  }
  if (details.cancelled) {
    return JSON.stringify(
      { cancelled: true, reason: details.reason, answers: details.answers },
      null,
      2,
    );
  }
...
function nonInteractiveResult(questions: AskUserQuestionItem[]) {
  ...
  return {
    content: [
      {
        type: "text" as const,
        text: `UI unavailable: ${details.reason}\n${formatDetailsForContent(details)}`,
      },
    ],
    details,
  };
...
function cancelledResult(
  ...
  return {
    content: [
      {
        type: "text" as const,
        text: `AskUserQuestion cancelled: ${reason}\n${formatDetailsForContent(details)}`,
      },
    ],
    details,
  };
...
class AskUserQuestionDialog extends Container implements Focusable {
  ...
  handleInput(data: string): void {
    ...
    if (customFocused) {
      ...
      this.updateCurrentState(
        setCustomText(question, this.currentState(), this.customAnswerInput.getValue()),
      );
      this.statusMessage = "";
      this.refresh();
    }
  ...
  private renderQuestionTabs(): void {
    const tabs = this.questions.map((question, index) => {
      ...
      return this.theme.fg(
        hasSelection(question, this.navigation.states[index]) ? "success" : "muted",
        label,
      );
    });
  ...
  private renderOptions(question: AskUserQuestionItem, state: SelectionState): void {
    for (let index = 0; index < question.options.length; index++) {
      ...
      const cursor =
        focused && this.focusState.mode === "options" ? this.theme.fg("accent", ">") : " ";
      ...
      const editing =
        this.focusState.mode === "choice-note" && this.focusState.activeChoiceNoteIndex === index;
  ...
  private renderChoiceNote(
    question: AskUserQuestionItem,
    state: SelectionState,
    optionIndex: number,
  ): void {
    const editing =
      this.focusState.mode === "choice-note" &&
      this.focusState.activeChoiceNoteIndex === optionIndex;
    if (editing) {
  ...
  private updateCurrentState(state: SelectionState): void {
    this.navigation = setQuestionState(
      this.questions,
      this.navigation,
      this.navigation.currentQuestionIndex,
      state,
    );
  }
    ...
    if (this.focusState.mode === "choice-note" && this.focusState.activeChoiceNoteIndex !== null) {
      this.noteInput.setValue(
        getChoiceNote(
          ...
          this.currentState(),
          this.focusState.activeChoiceNoteIndex,
        ),
      );
    }
  }
  ...
  private switchQuestionTab(delta: number): void {
    ...
    if (targetIndex < 0 || targetIndex >= this.questions.length) {
      this.statusMessage =
        delta < 0 ? "Already at the first question tab." : "Already at the last question tab.";
      this.refresh();
      return;
    }
  ...
  private handleOptionListEnter(): void {
    const action = decideOptionListEnterAction(
      this.navigation.currentQuestionIndex,
  ...
  private submitIfComplete(): void {
    ...
    if (missingIndex >= 0) {
      this.navigation = moveQuestion(
        this.questions,
        this.navigation,
        missingIndex - this.navigation.currentQuestionIndex,
      );
      this.focusState = returnFocusToOptions();
      this.prepareCustomInputForFocus();
...
function startRemoteDialog(
  questions: AskUserQuestionItem[],
  ctx: ExecuteContextLike,
  signal?: AbortSignal,
): RemoteDialogHandle {
  ...
  const promise = new Promise<RemoteDialogResult>((resolve) => {
    ...
    if (!sessionFile) {
      settle({
        type: "unavailable",
        reason: "No pi session file is available for Pocket Pi routing.",
      });
      return;
    }
    if (signal?.aborted) {
...
async function connectRemoteDialog(args: {
  ...
  try {
    ...
    args.settle({
      type: "unavailable",
      reason: `Pocket Pi daemon handshake unavailable: ${errMsg(err)}`,
    });
    return;
  }
  if (args.isSettled()) return;

  const WebSocketCtor = (globalThis as unknown as { WebSocket?: new (url: string) => SocketLike })
    .WebSocket;
  if (!WebSocketCtor) {
  ...
  try {
    ws = new WebSocketCtor(
      `ws://127.0.0.1:${handshake.port}/?token=${encodeURIComponent(handshake.token)}`,
    );
  } catch (err) {
    args.settle({
      type: "unavailable",
      reason: `Pocket Pi daemon connection failed: ${errMsg(err)}`,
    });
    return;
  }
  if (args.isSettled()) {
  ...
  onSocket(ws, "open", () => {
    ...
    try {
      ...
      args.settle({
        type: "unavailable",
        reason: `Pocket Pi remote question send failed: ${errMsg(err)}`,
      });
    }
  });
  onSocket(ws, "message", (ev) => {
    ...
    if (frame.op === "ask_answer") {
      args.settle({
        type: "answered",
        states: remoteAnswerToStates(args.questions, frame.answers),
      });
      return;
    }
    if (frame.op === "ask_dismiss") {
      const reason =
        typeof frame.reason === "string" && frame.reason
          ? frame.reason
          : "Dismissed from Pocket Pi.";
      args.settle(
        remoteDismissIsUnavailable(reason)
  ...
  onSocket(ws, "close", () => {
    args.settle({
      type: "unavailable",
      reason: "Pocket Pi daemon connection closed before an answer.",
    });
  });
  onSocket(ws, "error", () => {
    args.settle({
      type: "unavailable",
      reason: "Pocket Pi daemon connection failed before an answer.",
    });
  });
}

function remoteAnswerToStates(
  questions: AskUserQuestionItem[],
  rawAnswers: unknown,
): SelectionState[] {
...
function remoteSelectedLabels(value: unknown): string[] {
  ...
  return values.filter(
    (item): item is string => typeof item === "string" && item !== CUSTOM_ANSWER_LABEL,
  );
}
...
function remoteDismissIsUnavailable(reason: string): boolean {
  ...
  return (
    lower.includes("no pocket pi client") ||
    lower.includes("all pocket pi clients") ||
    lower.includes("unavailable")
  );
}

async function readHandshake(): Promise<{ port: number; token: string }> {
  const parsed = JSON.parse(await readFile(HANDSHAKE_PATH, "utf8")) as {
    port?: unknown;
    token?: unknown;
  };
  if (
...
export default function AskUserQuestion(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    promptSnippet:
      "Ask the user a structured multiple-choice question and return selected option labels/custom answers plus per-choice notes.",
    promptGuidelines: [
    ...
    async execute(
      _toolCallId,
      params: { questions: AskUserQuestionItem[] },
      signal,
      _onUpdate,
      ctx,
    ) {
    ...
    renderCall(args, theme, _context) {
      const questions = Array.isArray(args.questions)
        ? (args.questions as AskUserQuestionItem[])
        : [];
    ...
    renderResult(result, _options, theme, _context) {
      ...
      if (details.unavailable) {
        return new Text(
          theme.fg("warning", `UI unavailable: ${details.reason ?? "headless mode"}`),
          0,
          0,
        );
      }
      if (details.cancelled) {
        return new Text(
          theme.fg("warning", `Cancelled: ${details.reason ?? "user cancelled"}`),
          0,
          0,
        );
      }
